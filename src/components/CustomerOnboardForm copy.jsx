...
export default function CustomerOnboardForm({ currentData }) {
  ...
  const CustomerSchema = Yup.object().shape({
    ...
    Cust_Business_Type_ID: Yup.object().required('Business type is required'),
    Agent_ID: Yup.object().required('Agency is required'),
    End_Customer_Name_ID: Yup.object().required('End customer is required'),
    End_Cust_Business_Type_ID: Yup.object().required('End customer business type is required'),
    ...
    Cust_Per_of_Bus_ID: Yup.object().required('Business percentage is required'),
    Per_Export_Bus_ID: Yup.object().required('Export percentage is required'),
    Per_Of_bus_In_EUROPE: Yup.object().required('Europe business percentage is required'),
    Years_In_Bus_ID: Yup.object().required('Years in business is required'),
    Main_Export_Mrkt_Country_ID: Yup.object().required('Export market is required'),
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      Cust_Onboarding_Email: currentData?.WIC_Emial || '',
      Cust_Country_ID: countries?.find((x) => x.Country_ID === currentData?.WIC_Country_ID) || null,
      Cust_City_ID: allCities?.find((x) => x.City_ID === currentData?.WIC_City_ID) || null,
      Cust_Landline_No: currentData?.WIC_Phone || '',
      Cust_Business_Type_ID:
        businessType?.find((x) => x.CustBusinessType_ID === currentData?.WIC_Business_Type_ID) ||
        null,
  ...
  // Handle add business percentage by country
  const handleAddBusinessPercentageByCountry = () => {
    const newBusinessPercentageByCountry = {
      Business_Country_ID: null,
      Business_Percentage: null,
      Brand_Name: '',
      CreatedBy: userData?.userDetails?.userId || 1,
    ...
    setBusinessPercentageByCountry([
      ...businessPercentageByCountry,
      newBusinessPercentageByCountry,
    ]);
    setValue('businessPercentageByCountry', [
      ...values.businessPercentageByCountry,
      newBusinessPercentageByCountry,
    ]);
  };
  ...
  const InsertBussinessNumbersDtl = async (bussinessNumbersDtlData) => {
    try {
      ...
      formData.append(
        'Main_Export_Mrkt_Country_ID',
        bussinessNumbersDtlData?.Main_Export_Mrkt_Country_ID
      );
      ...
      formData.append('Business_License_File', bussinessNumbersDtlData?.Business_License_File);
  ...
  const InsertMasterData = async (data) => {
    if (currentData?.WIC_ID) {
      try {
        ...
        formData.append('Agent_ID', data?.Agent_ID?.AgentID);
        formData.append('Cust_Abb', data?.Cust_Abb);
        formData.append('Cust_Address1', data?.Cust_Address1);
        formData.append('Cust_Address2', data?.Cust_Address2);
        formData.append('Cust_Landline_No', data?.Cust_Landline_No);
        formData.append('Cust_ZipCode', data?.Cust_ZipCode);
        formData.append('Cust_Onboarding_Email', data?.Cust_Onboarding_Email);
        ...
        formData.append('Cust_Business_Type_ID', data?.Cust_Business_Type_ID?.CustBusinessType_ID);
        formData.append(
          'End_Cust_Business_Type_ID',
          data?.End_Cust_Business_Type_ID?.CustBusinessType_ID
        );
        formData.append('End_Customer_Name_ID', data?.End_Customer_Name_ID?.End_Cust_ID);
        formData.append('End_Cust_Country_ID', data?.End_Customer_Name_ID?.End_Cust_CountryID);
        formData.append('End_Cust_City_ID', data?.End_Customer_Name_ID?.End_Cust_CityID);
        ...
        if (response.status === 200) {
          ...
          const customerContactData = contacts.map((contact, index) => ({
            ...
            Contact_Type_ID: values?.contacts
              ...
              : 0,
            Contact_Person_Name: values?.contacts
              ? values?.contacts[index]?.Contact_Person_Name
              : '',
          ...
          const bussinessNumbersData = {
            ...
            Per_Export_Bus_ID: data?.Per_Export_Bus_ID?.PerOfBusinessInEroupID,
            ...
            Cust_Per_of_Bus_ID: data?.Cust_Per_of_Bus_ID?.PerOfBusinessInEroupID,
            isActive: true,
            CreatedBy: userData?.userDetails?.userId || 1,
            UpdatedBy: userData?.userDetails?.userId || 1,
            Org_ID: userData?.userDetails?.orgId,
          ...
          const bussinessNumbersDtlData = {
            ...
            Main_Export_Mrkt_Country_ID: data?.Main_Export_Mrkt_Country_ID.Country_ID,
        ...
        <Grid xs={12} md={12}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <RHFTextField name="Cust_Name" label="Name" inputProps={{ maxLength: 100 }} />
              ...
              <RHFTextField name="Cust_Landline_No" label="Contact No" />
              ...
              <RHFAutocomplete
                ...
                // value={activeAgents?.find(
                //   (option) => option?.AgentID === values?.Agent_ID?.AgentID
                // )}
              ...
              <RHFAutocomplete
                ...
                // value={allEndCustomers?.find(
                //   (option) => option?.End_Cust_ID === values?.End_Customer_Name_ID?.End_Cust_ID
                // )}
              ...
              {/* <RHFAutocomplete
                ...
                //   (option) =>
                //     option?.CustBusinessType_ID ===
                //     values?.End_Cust_Business_Type_ID?.CustBusinessType_ID
                // )}
                getOptionLabel={(option) => option?.BusinessType_Name}
              /> */}
              <AutocompleteWithAdd
        ...
        <Grid xs={12} md={12}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <Grid container spacing={1}>
                ...
                <Grid item xs={4}>
                  <RHFAutocomplete
                    ...
                    options={CAPUNIT}
                    // value={CAPUNIT?.find(
                    //   (option) => option?.Unit_ID === values?.Cust_Prod_Cap_Unit_ID?.Unit_ID
                    // )}
              ...
              <Grid container spacing={1}>
                <Grid item xs={8}>
                  <RHFTextField name="Cust_TurnoverPY" label="Turnover per year" type="number" />
                </Grid>
                <Grid item xs={4}>
                  <RHFAutocomplete
              ...
              <RHFTextField name="Remarks" label="Remarks" multiline minRows={3} />
            </Box>
          </Card>
        </Grid>
          ...
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <RHFAutocomplete
                ...
                options={noOfEmp}
                // value={noOfEmp?.find(
                //   (option) => option?.NO_Of_Emp_ID === values?.No_Of_Employees_ID?.NO_Of_Emp_ID
                // )}
                getOptionLabel={(option) => option?.Total_No_Of_Employees}
              />
              <RHFAutocomplete
                name="Cust_Per_of_Bus_ID"
                label="% of Business"
                placeholder="Choose an option"
                fullWidth
                options={businessPercentage}
                // value={businessPercentage?.find(
                //   (option) =>
                //     option?.PerOfBusinessInEroupID ===
                //     values?.Cust_Per_of_Bus_ID?.PerOfBusinessInEroupID
                // )}
                getOptionLabel={(option) => option?.Percentage}
              />
              <RHFAutocomplete
                name="Per_Export_Bus_ID"
                ...
                //   (option) =>
                //     option?.PerOfBusinessInEroupID ===
                //     values?.Per_Export_Bus_ID?.PerOfBusinessInEroupID
                // )}
                getOptionLabel={(option) => option?.Percentage}
              />
              <RHFAutocomplete
                name="Per_Of_bus_In_EUROPE"
                label="% of Business in USA/Europe"
                placeholder="Choose an option"
                fullWidth
                options={businessPercentage}
                // value={businessPercentage?.find(
                //   (option) =>
                //     option?.PerOfBusinessInEroupID ===
                //     values?.Per_Of_bus_In_EUROPE?.PerOfBusinessInEroupID
                // )}
                getOptionLabel={(option) => option?.Percentage}
              />
              <RHFAutocomplete
                ...
                // value={yearsInBusiness?.find(
                //   (option) =>
                //     option?.Cust_YearinBusinessID === values?.Years_In_Bus_ID?.Cust_YearinBusinessID
                // )}
                getOptionLabel={(option) => option?.Cust_YearInBusiness}
              />

              <RHFAutocomplete
                name="Main_Export_Mrkt_Country_ID"
                    ...
                    <Table>
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell sx={{ minWidth: 200 }}>Brand</TableCell>
            ...
            >
              ...
              >
                <RHFAutocomplete
                  ...
                  //   (option) =>
                  //     option?.Document_Type_ID ===
                  //     values?.certificateData?.CertificatePatentType?.Document_Type_ID
                  // )}
