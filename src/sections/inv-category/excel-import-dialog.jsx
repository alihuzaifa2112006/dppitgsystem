...
export default function UploadExcelDialog({
  ...
  const InsertVendor = useCallback(
    async (fileData) => {
      ...
      try {
        // check if the ClassID and Inv_Cat_Name exists in the table, if yes then exclude them from the fileData
        const filteredData2 = fileData.filter(
          (item) =>
            !tableData.some(
              (existingItem) =>
                existingItem.ClassID.toLowerCase().trim() === item.ClassID.toLowerCase().trim() &&
                existingItem.Inv_Cat_Name.toLowerCase().trim() ===
                  item.Inv_Cat_Name.toLowerCase().trim()
            )
        );
        ...
        const modifiedVendorData = filteredData2.map((X) => ({
          ...
          // ColorFamilyID: X?.ColorFamilyID || 0,
        }));
        const response = await Post(`upload/InvCategoryExcel`, modifiedVendorData);
  ...
  const handleFileChange = async () => {
    ...
    reader.onload = async (m) => {
      ...
      const customHeaders = ['ClassID', 'Inv_Cat_Name', 'Symbol'];
          ...
          <Grid container>
            <Grid xs={12} md={12}>
              <DialogContent>
                ...
                <a href={`${APP_API}export/InvCategory`} download>
                  <Button
