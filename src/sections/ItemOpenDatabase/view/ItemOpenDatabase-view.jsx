...
import ItemOpenDatabaseTableRow from '../ItemOpenDatabase-table-row';
import ItemOpenDatabaseTableToolbar from '../ItemOpenDatabase-toolbar';
import ItemOpenDatabaseTableFiltersResult from '../ItemOpenDatabase-filters-result';
...
export default function ItemOpenDatabaseListView() {
  ...
  const FetchItemOpenDatabaseData = useCallback(async () => {
    try {
      const response = await Get(
        `GetAllInvItemOpenDatabaseing?orgId=${userData?.userDetails?.orgId}&branchId=${userData?.userDetails?.branchID}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchItemOpenDatabaseData()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchItemOpenDatabaseData]);
  ...
  const moveToEditForm = (e) => {
    navigate(paths.dashboard.InventoryManagement.ItemOpenDatabase.edit(e));
  };
  // Approval Functions
  const moveToApprovalForm = (e) => {
    navigate(paths.dashboard.InventoryManagement.ItemOpenDatabase.approval(e));
  };
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Item Entry', href: paths.dashboard.InventoryManagement.ItemOpenDatabase.root },
              { name: 'List' },
            ]}
            action={
              <Button
          ...
          <Card>
            ...
            {canReset && (
              <ItemOpenDatabaseTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
                //
