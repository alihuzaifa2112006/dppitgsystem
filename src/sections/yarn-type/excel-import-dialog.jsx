...
export default function UploadExcelDialog({
  ...
  const InsertVendor = useCallback(
    async (fileData) => {
      ...
      try {
        // check if the Yarn_Type and Yarn_Code exists in the table, if yes then exclude them from the fileData
        const filteredData2 = fileData.filter(
          (item) =>
            !tableData.some(
              (existingItem) =>
                existingItem.Yarn_Type.toLowerCase().trim() ===
                  item.Yarn_Type.toLowerCase().trim() &&
                existingItem.Yarn_Code.toLowerCase().trim() === item.Yarn_Code.toLowerCase().trim()
            )
        );
        ...
        const response = await Post(`yarntypebulkUpload`, modifiedVendorData);

        if (response.status === 200) {
          enqueueSnackbar('Yarn Types added successfully!', { variant: 'success' });
        }
        uploadClose();
        FetchUpdatedData();
      } catch (error) {
  ...
  const handleFileChange = async () => {
    ...
    reader.onload = async (m) => {
      ...
      const customHeaders = ['Yarn_Code', 'Yarn_Type'];
          ...
          <Grid container>
            <Grid xs={12} md={12}>
              <DialogContent>
                ...
                <a href="https://apicyclo.scmcloud.online/Uploads/YarnTypeBulkUpload.xlsx" download>
                  <Button
