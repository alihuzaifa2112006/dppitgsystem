...
import GoodsRecievedConfirmationTableToolbar from '../GoodsRecievedConfirmation-toolbar';
import GoodsRecievedConfirmationTableFiltersResult from '../GoodsRecievedConfirmation-filters-result';
...
export default function GoodsRecievedConfirmationListView() {
 ...
 const flattenRequestData = (requests) => requests.flatMap(request => 
  ...
  const FetchGoodsRecievedConfirmationData = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchGoodsRecievedConfirmationData()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchGoodsRecievedConfirmationData]);
  ...
  const moveToEditForm = (e) => {
    // navigate(paths.dashboard.Production.GoodsRecievedConfirmation.edit(e));
  };

  const DeleteDetailTableRow = async (id) => {
    try {
      await Delete(`GoodsRecievedConfirmation/delete/${id}/${userData?.userDetails?.userId}`);
    ...
    <>
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
            ...
            action={
              ...
              >
                <Button
                  ...
                  href={paths.dashboard.Production.GoodsRecievedConfirmation.new}
                  variant="contained"
          ...
          <Card>
            <GoodsRecievedConfirmationTableToolbar
              filters={filters}
            ...
            {canReset && (
              <GoodsRecievedConfirmationTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
                ...
                >
                  ...
                  <TableBody>
                    {dataFiltered
                      ...
                      .map((row) => (
                        <GoodsRecievedConfirmationTableRow
