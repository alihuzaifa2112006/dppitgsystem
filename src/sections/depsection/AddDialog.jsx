...
export default function AddDialog({ uploadClose, uploadOpen, tableData }) {
  ...
  const PostDepsectionData = async (PostData) => {
    try {
      await Post('AddSection', PostData).then(async (res) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (
      tableData.some(
        (item) =>
          item.SectionName.toLowerCase() === data.SectionName.toLowerCase() &&
          item.DPT_ID === data.Dpt_ID.Dpt_ID
      )
    ) {
    ...
    try {
      ...
      await PostDepsectionData(dataToSend);
    } catch (error) {
      console.error(error);
    }
  });
