...
// Column widths for Independent Purchase Type
const independentColumnWidths = ['8%', '12%', '12%', '12%', '20%', '8%', '10%', '10%', '8%'];

// Column widths for Referenced Purchase Type (PurchaseTypeID === 2)
const referencedColumnWidths = ['10%', '18%', '15%', '20%', '10%', '12%', '12%', '8%'];
...
const PRI = ({ PRRequestID }) => {
  ...
  // Check if PurchaseType is Referenced (PurchaseTypeID === 2)
  const isReferenced = data.PurchaseTypeID === 2;
        ...
        <Page size="A3" style={styles.page}>
          ...
          <View style={{ border: 1, borderColor: '#000', marginTop: 10 }} wrap>
              ...
              {isReferenced
                ? [
                  ...
                  <Text
                    key={col}
                    style={{
                      width: referencedColumnWidths[i],
                      padding: 4,
                      fontFamily: 'Roboto-Bold',
                      fontSize: 9,
                      borderRight: i !== 7 ? 1 : 0,
                      borderColor: '#000',
                      textAlign: i > 4 ? 'center' : 'center',
                    }}
                  >
                    {col}
                  </Text>
                ...
                : [
                  ...
                  <Text
                    key={col}
                    style={{
                      width: independentColumnWidths[i],
                      padding: 4,
                      fontFamily: 'Roboto-Bold',
                      fontSize: 9,
                      borderRight: i !== 8 ? 1 : 0,
            ...
            {data.Details.map((item, idx) => {
              // Handle MRP field - could be direct or nested
              const mrpNo = item.MRPNo || item.MRP?.MRPNo || item.MRP_No || '-';
                ...
                <View key={idx} style={{ flexDirection: 'row', borderTop: 1, borderColor: '#000' }}>
                  {isReferenced
                    ? [
                      ...
                      item.UOMName || '-',
                      fNumber(item.PRUnitPrice) || '0.00',
                      fNumber(item.PRQTY) || '0.00',
                      fNumber((item.PRQTY || 0) * (item.PRUnitPrice || 0)),
                    ].map((val, i) => (
                      <Text
                        key={i}
                        style={{
