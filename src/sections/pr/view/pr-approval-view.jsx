...
import PrApprovalForm from '../pr-approval';
import { useEffect, useMemo, useState } from 'react';
import { Get } from 'src/api/apibasemethods';

// ----------------------------------------------------------------------

export default function PrApprovalView({ urlData }) {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const formatedData = {
        ...
        PRNo: response.data.Data[0]?.ApplyForReapproval ? `${response.data.Data[0]?.PRNo}-R${response.data.Data[0]?.HistoryCount}` : response.data.Data[0]?.PRNo,
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      <CustomBreadcrumbs
        heading={` ${
          isReapproval ? `Re-open PR #${currentData?.PRNo || ''}` : `Approval PR #${currentData?.PRNo || ''}`
        }`}
        links={[
          {
            name: 'Home',
      ...
      {currentData && <PrApprovalForm currentData={currentData} isReapproval={isReapproval} />}
    </Container>
  );
}

PrApprovalView.propTypes = {
