...
import PrTableToolbar from '../pr-toolbar';
import PrTableFiltersResult from '../pr-filters-result';
import axios from 'axios';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'PRNo', label: 'PR No.', minWidth: 145 },
...
export default function PrListView() {
  ...
  //       `GetPRLIST?OrgID=${userData?.userDetails?.orgId}&BranchID=${userData?.userDetails?.branchID}`
  //     );
  //     const updatedData = response.data.PendingPRList.map((item) => ({
  //       ...item,
  ...
  const FetchPrData = useCallback(async () => {
    try {
      ...
      try {
        const res = await Get(
          `getPRListforApproval?OrgID=${userData?.userDetails?.orgId}&BranchID=${userData?.userDetails?.branchID}&UserID=${userData?.userDetails?.userId}&DOCID=2`
        );
      ...
      const updatedDataWithToBeApproved = updatedData.map((item) => {
        const isMatching = data.some((resItem) => resItem?.PRID === item.PRID);
        return {
          ...item,
  ...
  const moveToEditForm = (e) => {
    navigate(paths.dashboard.procurement.pr.edit(e));
  };
  const moveToViewForm = (e) => {
    navigate(paths.dashboard.procurement.pr.pdf(e));
  };
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Proforma Invoice', href: paths.dashboard.procurement.pr.root },
              { name: 'List' },
            ]}
            action={
              <Button
                component={RouterLink}
                href={paths.dashboard.procurement.pr.new}
                variant="contained"
                    ...
                    {dataFiltered
                      ...
                      .map((row) => (
                        <PrTableRow
                          key={row?.PRID}
                          row={row}
                          selected={table.selected.includes(row?.PRID)}
                          onEditRow={() => moveToEditForm(row?.PRID)}
                          onViewRow={() => moveToViewForm(row?.PRID)}
                        />
                      ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.PRNo.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
