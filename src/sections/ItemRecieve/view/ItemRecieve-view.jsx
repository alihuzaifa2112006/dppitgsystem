...
import ItemOpenTableRow from '../ItemRecieve-table-row';
import ItemOpenTableToolbar from '../ItemRecieve-toolbar';
import ItemOpenTableFiltersResult from '../ItemRecieve-filters-result';
...
const TABLE_HEAD = [
  { id: 'Material_Code', label: 'Receive Order', minWidth: 180 },
  { id: 'ClassName', label: 'Order Receive', minWidth: 180 },
...
export default function ItemOpenListView() {
  ...
  const FetchItemOpenData = useCallback(async () => {
    try {
      const response = await Get(
        `GetAllInvItemOpening?orgId=${userData?.userDetails?.orgId}&branchId=${userData?.userDetails?.branchID}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchItemOpenData()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchItemOpenData]);
  ...
  const moveToEditForm = (e) => {
    navigate(paths.dashboard.InventoryManagement.ItemOpen.edit(e));
  };
  // Approval Functions
  const moveToApprovalForm = (e) => {
    navigate(paths.dashboard.InventoryManagement.ItemOpen.approval(e));
  };
