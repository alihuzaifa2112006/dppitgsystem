...
export default function AddDialog({ uploadClose, uploadOpen, tableData }) {
  ...
  const PostRoleData = async (PostData) => {
    try {
      await Post('AddRMRole', PostData).then(async (res) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (
      tableData.some(
        (item) =>
          item.Name.toLowerCase() === data.Name.toLowerCase() && item.DPT_ID === data.Dpt_ID?.Dpt_ID
      )
    ) {
