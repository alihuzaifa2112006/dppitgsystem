...
// import ProductRpdf from '../ProductRequest';
import DepartmentalRequestPDF from '../ProductRequest';
...
export default function ProductRpdfView({ urlData }) {
  ...
  const GetPDFData = useCallback(async () => {
    ...
    try {
      ...
      const response = await Get(`GetReqByID?reqId=${ReqID}&orgId=${orgId}&branchId=${branchId}`);

      const fetchedData = response.data?.Data?.[0] || response.data;

      setCurrentData(fetchedData);
    } catch (err) {
      console.error(err);
      setError('Failed to load Product Request data');
      enqueueSnackbar('Failed to load Product Request', { variant: 'error' });
    } finally {
      setIsLoading(false);
    }
  }, [ReqID, userData]);
    ...
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      <CustomBreadcrumbs
        ...
        links={[
          ...
          { name: 'Departmental Request', href: paths.dashboard.Production.ProductRequest.root },
          { name: 'PDF' },
        ]}
        sx={{ mb: { xs: 3, md: 5 } }}
      />
      ...
      {isLoading ? (
        ...
        <Box display="flex" justifyContent="center" alignItems="center" minHeight="400px">
          <Typography variant="h6" color="text.secondary">
            No Product Request Report Found
          </Typography>
        </Box>
      )}
    </Container>
