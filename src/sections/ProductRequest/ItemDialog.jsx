...
export default function ItemDialog({
  ...
  const columnDefs = [
    {
      ...
      cellRenderer: (params) => (
        <Checkbox
          sx={{ p: 0 }}
          size="small"
          disabled={params.data.issueRemainingQty <= 0}
  ...
  const handleSelectAll = (isSelected) => {
    if (isSelected) {
      const filteredItems = itemData.filter((item) => item.issueRemainingQty > 0);
  ...
  const fetchAllItemData = useCallback(async () => {
    if (selectedItem?.ItemID) {
      ...
      if (response?.status === 200) {
        ...
        })).filter((item) => item.StoreID === values?.Request?.StoreID);
        if (dataWithKey.length === 0) {
          enqueueSnackbar(`${selectedItem?.ItemDescription} not available in ${values?.Request?.StoreName}`, { variant: 'warning' });
        }
    ...
    values?.Request?.StoreName,
    values?.Request?.StoreID,
    setItemData,
  ]);
