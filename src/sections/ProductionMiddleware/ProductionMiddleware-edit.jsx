...
export default function ProductionMiddlewareEditForm({ currentData }) {
  ...
  const [ProductionMiddlewareDetails, setProductionMiddlewareDetails] = useState([]);
  const [editingIndex, setEditingIndex] = useState(null);
  const [customers, setCustomers] = useState([]);
  const [isLoading, setLoading] = useState(true);
  const [allItems, setAllItems] = useState([]);

  const NewProductionMiddlewareSchema = Yup.object().shape({
    // ClassID
  ...
  const methods = useForm({
    resolver: yupResolver(NewProductionMiddlewareSchema),
    defaultValues: {
      ClassID: null,
      Inv_Cat_Name: null,
  ...
  const onsubmitie = handleSubmit(async (data) => {
    ...
    try {
      ...
      router.push(paths.dashboard.Production.Planning.ProductionMiddleware.root);
    } catch (error) {
  ...
  const notFound = !ProductionMiddlewareDetails.length;
...
ProductionMiddlewareEditForm.propTypes = {
