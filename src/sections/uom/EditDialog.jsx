...
export default function UomEditDialog({ uploadClose, uploadOpen, row, tableData }) {
  ...
  const NewUomSchema = Yup.object().shape({
    Uom_Name: Yup.string()
      ...
      .min(3, 'Uom Name must be at least 3 characters long')
      .max(100, 'Uom Name must be less than or equal to 100 characters'),
    // .matches(/^[a-zA-Z\s]+$/, 'Uom Name must only contain letters and spaces'),
  });
  ...
  const PutUomData = async (PutData) => {
    try {
      await Put(`uom/${row?.Uom_ID}`, PutData).then(async (res) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (
      tableData.some(
        (item) => item.Uom_Name === data.Uom_Name && row.Uom_Name !== data.Uom_Name
      )
    ) {
      enqueueSnackbar('Uom already exists', { variant: 'error' });
      return;
    }
    try {
      const dataToSend = {
      ...
      >
        ...
        <DialogContent sx={{ py: 3 }}>
          <FormProvider methods={methods} onSubmit={onSubmit}>
            ...
            >
              <RHFTextField name="Uom_Name" label="Uom Name" />
            </Box>
            <Box
              display="flex"
              alignItems="center"
