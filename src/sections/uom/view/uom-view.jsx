...
import UomTableRow from '../uom-table-row';
import UomTableToolbar from '../uom-toolbar';
import UomTableFiltersResult from '../uom-filters-result';
import { alpha, Tab, Tabs } from '@mui/material';
import Label from 'src/components/label';
import UomDialog from '../AddDialog';
import UomEditDialog from '../EditDialog';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'UOMName', label: 'Unit', minWidth: 160 },
  // { id: 'UOMSymbol', label: 'Symbol', minWidth: 140 },
...
export default function UomListView() {
  ...
  const FetchUom = useCallback(async () => {
    try {
      const response = await Get(`GetAllActiveUOM?OrgID=${userData?.userDetails?.orgId}&BranchID=${userData?.userDetails?.branchID}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchUom(), FetchCity()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchUom, FetchCity]);
  ...
  const uniqueUom_Name = [...new Set(tableData.map((tb) => tb.Uom_Types))];
  ...
  const moveToEditForm = (e) => {
    if (allCities.find((city) => city.Uom_ID === e.Uom_ID)) {
      enqueueSnackbar('This Unit of Measure is in use!', { variant: 'error' });
      return;
    }
  ...
  const DeleteDetailTableRow = async (row) => {
    ...
    // if (allCities.some((city) => city.Uom_ID === row.Uom_ID)){
    //   enqueueSnackbar('This Uom is in use!', { variant: 'error' });
    //   return;
    // }
    try {
      await Delete(`DeleteUomType?UomTypeID=${row.Uom_TypeID}`);
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Unit of Measure', href: paths.dashboard.powertools.uom },
              { name: 'list' },
            ]}
            action={
              <Button
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        ...
        // yarn?.UOMSymbol.toLowerCase().indexOf(name.toLowerCase()) !== -1
  ...
  if (role.length) {
    inputData = inputData.filter((yarn) => role.includes(yarn?.UOMName));
  }
