...
import EndCustomerEdit from '../end-customer-edit';
import { APP_API_STORAGE } from 'src/config-global';

// ----------------------------------------------------------------------

export default function EndCustomerView({ urlData }) {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (res.status === 200) {
        const data = {
          ...res.data.Data,
          End_Cust_Logo: res.data.Data.End_CustLogoPath && {
            preview: `${APP_API_STORAGE}${res.data.Data.End_CustLogoPath}`,
            name: res.data.Data.End_CustLogoPath.split('/').pop(),
            type: 'image/png',
          },
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      ...
      {isLoading ? (
        ...
        <EndCustomerEdit currentData={currentData} />
      )}
    </Container>
  );
}

EndCustomerView.propTypes = {
