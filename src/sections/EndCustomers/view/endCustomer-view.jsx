...
import EndCustomerTableRow from '../end-customer-table-row';
import EndCustomerTableToolbar from '../end-customer-toolbar';
import EndCustomerTableFiltersResult from '../end-customer-filters-result';
...
const TABLE_HEAD = [
  { id: 'End_CustLogoPath', label: 'Logo', minWidth: 20, align: 'center' },
  { id: 'End_Cust_Name_Org', label: 'Main Buyer Name', minWidth: 140 },
  { id: 'End_Cust_Name', label: 'Name (Country Code)', minWidth: 140 },
...
export default function EndCustomerListView() {
  ...
  const handleDialogOpen = () => {
    navigate(paths.dashboard.customer.endCustomer.new);
  };
  ...
  const FetchEndCustomer = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchEndCustomer(), FetchAllCustomers()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchEndCustomer, FetchAllCustomers]);
  ...
  const moveToEditForm = (e) => {
    navigate(paths.dashboard.customer.endCustomer.edit(e));
  };
