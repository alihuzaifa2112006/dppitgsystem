...
import CardReportTableRow from './CardReport-table-row';
...
export default function CardReportCreateForm() {
  ...
  const NewCardReportSchema = Yup.object().shape({
    ...
    lineNo: Yup.object().nullable().required('Please select a Line No'),
    BlowroomNumber: Yup.object().nullable().required('Please select a Blowroom Rpt No.'),
    cardReportRows: Yup.array()
      .of(
        Yup.object().shape({
          CardType: Yup.object().nullable().required('Card Type is required'),
          Material: Yup.object().nullable().required('Material is required'),
          Qty: Yup.number()
            .nullable()
  ...
  // Reset function to clear grid rows when BlowroomNumber changes or is cleared
  const resetGridRows = useCallback(() => {
    setCardReportRows([{ id: 1 }]);
    setAllMaterialName([]); // Clear material list
  ...
  const validateTotalKgsSum = (rowIndex, newValue, fieldName) => {
    ...
    const aInput1 =
      fieldName === 'AInput1' ? parseFloat(newValue) || 0 : parseFloat(currentRow?.AInput1) || 0;
    const bInput1 =
      fieldName === 'BInput1' ? parseFloat(newValue) || 0 : parseFloat(currentRow?.BInput1) || 0;
    const cInput1 =
      fieldName === 'CInput1' ? parseFloat(newValue) || 0 : parseFloat(currentRow?.CInput1) || 0;
  ...
  // Fetch Blowroom Items when BlowroomNumber is selected
  useEffect(() => {
    const fetchBlowroomItems = async () => {
      ...
      // Reset rows whenever BlowroomNumber changes (new selection or cleared)
      resetGridRows();
      ...
      if (reportID && orgId && branchID) {
        ...
        // Clear materials when BlowroomNumber is cleared
        setAllMaterialName([]);
      }
    };
  ...
  const onSubmit = handleSubmit(
    async (data) => {
      ...
      const validRows = data.cardReportRows?.filter((row) => row.CardType && row.Material);

      if (!validRows || validRows.length === 0) {
        enqueueSnackbar(
          'Please add at least one valid production detail with Card Type and Material',
          { variant: 'error' }
        );
        return;
      }
      ...
      for (let i = 0; i < validRows.length; i += 1) {
        ...
        if (qty > 0) {
          ...
          if (totalSum > qty) {
            enqueueSnackbar(
              `Row ${i + 1}: Total KGS sum (${totalSum.toFixed(
                2
              )}) cannot exceed Qty (${qty.toFixed(2)})`,
              { variant: 'error' }
            );
            return;
          }
      ...
      const dataToSend = {
        ...
        BlwReportID: values.BlowroomNumber?.ReportID || 0,
      ...
      try {
        await Post(`ProductionCarding/Add`, dataToSend).then(async (res) => {
          if (res.status !== 200) {
            enqueueSnackbar(res.data.message, { variant: 'error' });
          } else {
            enqueueSnackbar('Carding Report Created Successfully!');
        ...
        console.error(error);
        enqueueSnackbar('Error creating carding report', { variant: 'error' });
      }
    },
    (validationErrors) => {
          ...
          <Card sx={{ p: 2 }}>
            ...
            >
              ...
              <RHFAutocomplete
                ...
                onchange={(newValue) => {
                  // Reset grid rows whenever BlowroomNumber value changes
                  resetGridRows();
                }}
              />
            </Box>
                        ...
                        <TableRow key={row.id}>
                          <TableCell align="center">
                            <RHFAutocomplete
                              ...
                              // value={values?.cardReportRows?.[index]?.PartID || null}
                            />
                          </TableCell>
                          <TableCell align="center">
                            <RHFAutocomplete
                              ...
                              getOptionLabel={(option) =>
                                option?.MaterialName || option?.Name || ''
                              }
                              isOptionEqualToValue={(option, value) =>
                                option?.MaterialID === value?.MaterialID || option?.ID === value?.ID
                              }
                              value={values?.cardReportRows?.[index]?.Material || null}
                              onchange={(newValue) => {
                                // Set Qty to TotalWeight when Material is selected
                                ...
                                endAdornment: (
                                  <InputAdornment position="end">
                                    <Typography variant="body2">
                                      {values?.cardReportRows[index]?.Material?.UOMName || ''}
                                    </Typography>
                                  </InputAdornment>
                                ),
                              }}
                          ...
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              fullWidth
                              size="small"
                              value={values?.cardReportRows?.[index]?.Speed || ''}
                            />
                          </TableCell>
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              fullWidth
                              size="small"
                              value={values?.cardReportRows?.[index]?.Gran || ''}
                            />
                          </TableCell>
                          ...
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              fullWidth
                              size="small"
                              value={values?.cardReportRows?.[index]?.AInput1 || ''}
                              InputProps={{
                                endAdornment: (
                                  <InputAdornment position="end">
                              ...
                              onchange={(e) => {
                                ...
                                // Validate sum doesn't exceed Qty
                                const validation = validateTotalKgsSum(
                                  index,
                                  e.target.value,
                                  'AInput1'
                                );
                          ...
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              fullWidth
                              size="small"
                              value={values?.cardReportRows?.[index]?.AInput2 || ''}
                              disabled
                              InputProps={{
                                readOnly: true,
                              }}
                          ...
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              fullWidth
                              size="small"
                              value={values?.cardReportRows?.[index]?.BInput1 || ''}
                              InputProps={{
                                endAdornment: (
                                  <InputAdornment position="end">
                                    <Typography variant="body2">{values?.cardReportRows[index]?.Material?.UOMName || ''}</Typography>
                          ...
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              fullWidth
                              size="small"
                              value={values?.cardReportRows?.[index]?.BInput2 || ''}
                              disabled
                              InputProps={{
                                readOnly: true,
                              }}
                          ...
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              fullWidth
                              size="small"
                              value={values?.cardReportRows?.[index]?.CInput1 || ''}
                              InputProps={{
                                endAdornment: (
                                  <InputAdornment position="end">
                                    <Typography variant="body2">{values?.cardReportRows[index]?.Material?.UOMName || ''}</Typography>
                          ...
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              size="small"
                              disabled
                              value={values?.cardReportRows?.[index]?.CInput2 || ''}
                              InputProps={{
                                readOnly: true,
                              }}
                            />
                          ...
                          <TableCell align="center">
                            <RHFTextField
                              ...
                              fullWidth
                              size="small"
                              value={values?.cardReportRows?.[index]?.Remarks || ''}
                            />
                          </TableCell>
                          <TableCell align="center">
                            <IconButton
                              onClick={() => {
                                if (cardReportRows.length > 1) {
                                  const filteredRows = cardReportRows.filter(
                                    (r) => r.id !== row.id
                                  );
