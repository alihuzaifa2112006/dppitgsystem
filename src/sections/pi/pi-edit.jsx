...
export default function PiEditForm({ currentData, isReapproval }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      CostFactor: allCostFactors.find((cf) => cf.CostFactorID === currentData?.CostFactorID) || null,
  ...
  const generateProductName = useMemo(
    () =>
      `${values?.Yarn_Type_ID?.Yarn_Code || ''} - ${values?.Yarn_Count_ID?.Yarn_Count_Name || ''
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const revisedData = {
      ...
      PINo: currentData?.PINo,
      Payment_TermID: data?.PaymentTerms?.Payment_term_ID,
      PIDate: data?.ValidFrom ? fDate(data.PIDate, 'yyyy-MM-dd') : null,
      ...
      CostFactorCharges: data?.CostFactor?.CostFactorID !== 1 ? data?.CostFactorCharges : 0,
      Org_ID: userData?.userDetails?.orgId,
      Details: piDetails.map((detail) => ({
        PIDtlID: detail?.PIDtlID || 0,
        ...
        DeliveryDueDate: detail?.DeliveryDueDate ? fDate(detail?.DeliveryDueDate, 'yyyy-MM-dd') : null,
    ...
    // check UnitPrice using  PIDtlID and fetchedPriceOfProducts
    const priceOfAnyProductHasChanged = piDetails.some((detail) => {
      const fetchedPrice = fetchedPriceOfProducts.find(
        (product) => product.PIDtlID === detail.PIDtlID
      );
      return fetchedPrice && fetchedPrice.UnitPrice !== detail.Unit_Price;
    });
    // const noOfProductsHasChanged = piDetails.length !== fetchedPriceOfProducts.length;
    try {
      if (isReapproval === true) {
        await Post(
          `ReapprovePI?PIID=${currentData?.PIID}&UserID=${userData?.userDetails?.userId}&hasPriceChange=0` // &hasPriceChange=${priceOfAnyProductHasChanged ? 1 : 0}
        );
      }
