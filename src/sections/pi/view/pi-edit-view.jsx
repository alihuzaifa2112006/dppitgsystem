...
import PiEditForm from '../pi-edit';
import { useEffect, useMemo, useState } from 'react';
import { Get } from 'src/api/apibasemethods';

// ----------------------------------------------------------------------

export default function PiEditView({ urlData }) {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const formatedData = {
        ...
        PINo: response.data.Data[0]?.ApplyForReapproval ? `${response.data.Data[0]?.PINo}-R${response.data.Data[0]?.HistoryCount}` : response.data.Data[0]?.PINo,
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      <CustomBreadcrumbs
        heading={` ${
          isReapproval ? `Re-open PI #${currentData?.PINo || ''}` : `Edit PI #${currentData?.PINo || ''}`
        }`}
        links={[
          {
            name: 'Home',
      ...
      {currentData && <PiEditForm currentData={currentData} isReapproval={isReapproval} />}
    </Container>
  );
}

PiEditView.propTypes = {
