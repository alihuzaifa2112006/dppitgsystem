...
export default function PiEditForm({ currentData }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      if (revisedData?.Details?.length > 0) await Post('AddRevisedProformaDetails', revisedData);
  ...
  const handleAddDetail = () => {
    ...
    if (editingIndex !== null) {
      ...
      const dataHasChanged =
        ...
        updatedDetails[editingIndex]?.DeliveryDueDate !== values?.DeliveryDueDate ||
