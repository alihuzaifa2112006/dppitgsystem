...
import { borderBottom, borderColor, color } from '@mui/system';
...
export default function RecipeEdit({ currentData }) {
  const { enqueueSnackbar } = useSnackbar();
  const navigate = useNavigate();
  const userData = JSON.parse(localStorage.getItem('UserData'));

  // State for dropdown options
  const [currentTab, setCurrentTab] = useState(currentData?.RecipeType);
  ...
  const defaultValues = useMemo(
    () => ({
      Recipe_Name: currentData?.RecipeName || '',
      customer: currentData?.customer || null,
      Blend: currentData?.Blend || null,
      RevisionNo: currentData.RevisionNo + 1 || 0,
      version: `v${currentData.RevisionNo + 1}`,
      swatch_color: currentData?.Swatch_Color || '',
      buyer: currentData?.buyer || null,
      LabTechnician: currentData?.LabTechnician || null,
      RecipeCreationDate: currentData?.CreationDate || null,
      yarntype: currentData?.yarnType || null,
      yarnCount: currentData?.yarnCount || null,
      Composition: currentData?.Composition || null,
      colorfamily: currentData?.colorFamily || null,
      goalcolor: currentData?.goalcolor || null,
      Remarks: currentData?.Remarks || null,
      // recipeItems: [...recipeItems],
      recipe_pic: currentData?.RecipePictureURL || null,
      swatch_pic_with_cyclo_yarn: currentData?.SwatchColorPictureURL || null,
      data_color_score_card: currentData?.DataColorScoreCardURL || null,
      customer_swatch_pic: currentData?.CustomerSwatchPictureURL || null,
      recipeItems:
        currentData?.Details.map((item) => ({
          ...item,
          lotName: { LotName: item.RmLotNo } || null,
        })) || [],
    }),
    [currentData]
  );
  ...
  useEffect(() => {
    if (currentData && Object.keys(defaultValues).length > 0) {
      methods.reset(defaultValues);
    }
  }, [defaultValues, methods, currentData]);

  const values = watch();

  const customerField = watch('customer');

  useEffect(() => {
    if (customerField !== null && allSamples.length > 0) {
      setValue('sample_serial_no', currentData?.sample || null);
    }
    // eslint-disable-next-line
  }, [customerField, allSamples]);
  ...
  const fetchCustomers = useCallback(async () => {
    try {
      const response = await Get(
        `Production/GetWalkInCustomers?Org_ID=${userData?.userDetails?.orgId}&Branch_ID=${userData?.userDetails?.branchID}`
      );
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      formData.append('RecipeID', currentData?.RecipeID || 0);
      ...
      if (data.recipe_pic) {
        const recipeFile = data.recipe_pic instanceof File ? data.recipe_pic : data.recipe_pic;
        formData.append('RecipePicture', recipeFile);
      }

      if (data.swatch_pic_with_cyclo_yarn) {
        const finalColorFile =
      ...
      data.recipeItems.forEach((item, index) => {
        formData.append(`Details[${index}][RecipeDtlID]`, item?.RecipeDtlID || 0);
        ...
        if (item?.colorPicture?.file) {
          ...
          formData.append(`Details[${index}][ColorPicture_URL]`, item.colorPicture);
        }
      });

      // Submit the form data
      const response = await Put('Production/UpdateRecipe', formData, {
        headers: {
                    ...
                    <Stack>
                      ...
                      {values?.recipe_pic ? (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          ...
                          >
                            {typeof values?.recipe_pic === 'string'
                              ? values?.recipe_pic?.split('/').pop()
                      ...
                      <Stack>
                        ...
                        {values?.customer_swatch_pic ? (
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                            ...
                            >
                              {typeof values?.customer_swatch_pic === 'string'
                                ? values?.customer_swatch_pic?.split('/').pop()
                    ...
                    <Stack>
                      ...
                      {values?.swatch_pic_with_cyclo_yarn ? (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          ...
                          >
                            {typeof values?.swatch_pic_with_cyclo_yarn === 'string'
                              ? values?.swatch_pic_with_cyclo_yarn?.split('/').pop()
                    ...
                    <Stack>
                      ...
                      {values?.data_color_score_card ? (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          ...
                          >
                            {typeof values?.data_color_score_card === 'string'
                              ? values?.data_color_score_card?.split('/').pop()
