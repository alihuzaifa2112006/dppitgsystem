...
// const fetchPRData = async (userData, POID) => {
//   try {
//     const response = await Get(
//       `getProformaInvoicesAndDetails?OrgID=${userData?.userDetails?.orgId}&BranchID=${userData?.userDetails?.branchID}&PRID=${POID}`
//     );
...
export default function PoPDFView({ urlData }) {
  ...
  //   if (!urlData?.POID) return;

  //   setIsLoading(true);
  //   try {
  //     const [prClauses, rawPrData] = await Promise.all([
  //       getAllClauses(2),
  //       fetchPRData(userData, urlData.POID),
  ...
  //     setIsLoading(false);
  //   }
  // }, [urlData?.POID, userData, getAllClauses]);
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      <CustomBreadcrumbs
        ...
        //     href={`${paths.dashboard.procurement.po.edit(urlData?.POID)}&${generateRandomString()}`}
        //     variant="contained"
      ...
      {/* {isLoading ? (
        ...
        currentData &&  */}
        <PoReport currentData={currentData} poID={urlData?.POID} />
      {/* )} */}
    </Container>
  );
}
