  ...
  const onSubmit = handleSubmit(async (data) => {
    const payload = {
      ...
      BankDetails: bankDetails.map((bank, index) => ({
        Sup_Bank_Name: values?.bankDetails[index]?.Supp_Acc_Bank,
        Sup_Acc_Branch: values?.bankDetails[index]?.Supp_Acc_Branch,
        Sup_Acc_Title: values?.bankDetails[index]?.Supp_Acc_Title,
        Sup_Acc_No: values?.bankDetails[index]?.Supp_Acc_No,
        Sup_Acc_IBAN: values?.bankDetails[index]?.Supp_Acc_IBAN,
        IsActive: true,
        CreatedBy: userData?.userDetails?.userId || 1,
        UpdatedBy: userData?.userDetails?.userId || 1,
        Branch_ID: userData?.userDetails?.branchID || 1,
      ...
      InventoryTypes: values?.Inv_type?.map((inv) => ({
    ...
    await Post(`AddRMSuppliers`, payload)
      .then((response) => {
      ...
      .catch((error) => {
        enqueueSnackbar('SError adding supplier', { variant: 'error' });
            ...
            >
              ...
              <RHFAutocomplete
                ...
                onChange={(event, newValue) => {
                  const hasDirect = newValue.some((buyer) => buyer.AttributeId === 1);
                  let updatedValue = newValue;

                  if (hasDirect && newValue.length > 1) {
                    updatedValue = newValue.filter((buyer) => buyer.AttributeId === 1);
                  }
                  setValue('Inv_type', updatedValue, { shouldValidate: true });
                }}
              />
            </Box>
          </Card>
