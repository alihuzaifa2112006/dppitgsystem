...
const ItemVoucherPDFOrg = ({ currentData }) => {
  ...
  const totalQuantity = Details.reduce((sum, item) => sum + (parseFloat(item.TransferQty) || 0), 0);
  const uom = Data.UOMName || (Details.length > 0 ? Details[0].UOMName : '') || '';
        ...
        <Page size={[865, 1000]} style={styles.page}>
          ...
          <View style={{ border: 1, borderColor: '#000', marginTop: 10 }} wrap>
            ...
            <View style={{ flexDirection: 'row', backgroundColor: '#f0f0f0' }}>
              {['#', 'Item Code', 'Item Description', 'Types', 'Transfer Qty', 'Remaining Qty', 'UOM'].map((col, i) => (
                <Text
                  key={col}
                  style={{
                    width: ['6%', '14%', '38%', '12%', '14%', '14%', '8%'][i],
                    padding: 4,
                    fontFamily: 'Roboto-Bold',
                    fontSize: 8,
                    borderRight: i !== 6 ? 1 : 0,
            ...
            {Details.length === 0 ? (
              <View style={{ flexDirection: 'row', borderTop: 1, borderColor: '#000', padding: 8 }}>
                <Text style={{ fontSize: 9, width: '100%', textAlign: 'center' }}>No details</Text>
              </View>
            ) : (
              Details.map((item, idx) => (
                <View key={item.VOUDTLID || idx} style={{ flexDirection: 'row', borderTop: 1, borderColor: '#000' }}>
                  <Text style={{ width: '6%', padding: 4, fontSize: 8, borderRight: 1, borderColor: '#000', textAlign: 'center' }}>
                    {idx + 1}
                  </Text>
                  <Text style={{ width: '14%', padding: 4, fontSize: 8, borderRight: 1, borderColor: '#000' }}>
                    {item.ItemCode || '-'}
                  </Text>
                  <Text style={{ width: '38%', padding: 4, fontSize: 8, borderRight: 1, borderColor: '#000' }}>
                    {item.ItemDescription || '-'}
                  </Text>
                  <Text style={{ width: '12%', padding: 4, fontSize: 8, borderRight: 1, borderColor: '#000', textAlign: 'center' }}>
                    {item.Types || '-'}
                  </Text>
                  <Text style={{ width: '14%', padding: 4, fontSize: 8, borderRight: 1, borderColor: '#000', textAlign: 'right' }}>
                    {fNumber(item.TransferQty) ?? '-'}
                  </Text>
                  <Text style={{ width: '14%', padding: 4, fontSize: 8, borderRight: 1, borderColor: '#000', textAlign: 'right' }}>
                    {fNumber(item.RemainingQty) ?? '-'}
                  </Text>
                  <Text style={{ width: '8%', padding: 4, fontSize: 8, textAlign: 'center' }}>
                    {item.UOMName || Data.UOMName || '-'}
                  </Text>
                </View>
              ))
            )}
          ...
          <View style={styles.totalBox}>
            ...
            >
              ...
              <Text style={{ ...styles.totalText, textAlign: 'right' }}>
                {fNumber(Data.TotalQty ?? totalQuantity) ?? 0} {uom}
              </Text>
            </View>
          </View>
...
ItemVoucherPDFOrg.propTypes = {
  currentData: PropTypes.shape({
    ...
    Data: PropTypes.shape({
      ...
      PDONO: PropTypes.string,
      SHIFID: PropTypes.number,
      ...
      Details: PropTypes.arrayOf(
        PropTypes.shape({
          VOUDTLID: PropTypes.number,
          ItemID: PropTypes.number,
          ItemCode: PropTypes.string,
          ItemDescription: PropTypes.string,
          TransferQty: PropTypes.number,
