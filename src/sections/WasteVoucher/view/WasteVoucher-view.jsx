...
import WasteVoucherTableRow from '../WasteVoucher-table-row';
import WasteVoucherTableToolbar from '../WasteVoucher-toolbar';
import WasteVoucherTableFiltersResult from '../WasteVoucher-filters-result';
...
export default function WasteVoucherListView() {
 ...
 const flattenRequestData = (requests) => requests.flatMap(request => 
  ...
  const FetchWasteVoucherData = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchWasteVoucherData()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchWasteVoucherData]);
  ...
  const moveToEditForm = (e) => {
    // navigate(paths.dashboard.Production.WasteVoucher.edit(e));
  };

  const DeleteDetailTableRow = async (id) => {
    try {
      await Delete(`WasteVoucher/delete/${id}/${userData?.userDetails?.userId}`);
      ...
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
            ...
            action={
              ...
              >
                <Button
                  ...
                  href={paths.dashboard.Production.WasteVoucher.new}
                  variant="contained"
          ...
          <Card>
            ...
            {canReset && (
              <WasteVoucherTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
