...
export default function OpportunityApprovalForm({ currentData }) {
  ...
  const [opportunityDetails, setOpportunityDetails] = useState(currentData?.OppProduct || []);
  ...
  const defaultValues = useMemo(
    () => ({
      OpportunityName: currentData?.OpportunityName || '',
      ...
      OpportunityDate: currentData?.OpportunityDate || null,
      EndDate: currentData?.EndDate || null,
      PaymentTerms:
        allPaymentTerms.find((x) => x.Payment_term_ID === currentData?.PaymentTermID) || null,
      KAM: allKAMs?.find((kam) => kam.UserID === currentData?.KAM) || null,
      UOM: allUOM?.find((uom) => uom.UOM_ID === currentData?.OppProduct[0]?.UOMID) || null,
    }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [currentData, customers, allKAMs, allUOM]
  );
  ...
  const PostOpportunityMasterData = async (opData) => {
    try {
      await Put('UpdateOpportunityWithProducts', opData).then(async (res) => {
        if (res.status === 200) {
  ...
  const SendApproval = async (yesORno) => {
    ...
    const dataToSend = {
      Opportunity: {
        OpportunityID: currentData?.OpportunityID,
        ...
        PaymentTermID: values?.PaymentTerms?.Payment_term_ID,
      ...
      Products: opportunityDetails
        ?.filter((detail) => detail?.updated)
        ?.map((detail) => ({
          OpportunityProductID: detail?.OpportunityProductID || 0,
          OpportunityID: currentData?.OpportunityID,
    ...
    try {
      ...
      const respones = await Post(`UpdateOpportunityApproval`, {
        OpportunityID: currentData?.OpportunityID,
      ...
      if (respones?.status === 200) {
        enqueueSnackbar('Opportunity Approved Successfully', { variant: 'success' });
        const generatedLink = `${APP_URL}${paths.dashboard.transaction.opportunity.approval(
          currentData?.OpportunityID
        )}`;
        const emailData = {
          OpportunityID: currentData?.OpportunityName,
    ...
    <body>
        <div class="container">
            <h2 class="header">Opportunity Approved</h2>

            <p>Dear ${currentData?.KAM_Name},</p>

            <p>The following opportunity has been approved and is ready for processing:</p>

            <div class="details">
                <p><strong>Opportunity Name:</strong> ${currentData?.OpportunityName}</p>
                <p><strong>Approved By:</strong> ${userData?.userDetails?.userName}</p>
                <p><strong>Approval Date:</strong> ${fDate(new Date())}</p>

            </div>

            <p>You can view the details of the approved opportunity below:</p>
            ...
            <table cellspacing="0" cellpadding="0" style="margin-bottom: 10px;">
                <tr>
                    <td align="center" width="200" height="40" bgcolor="#5e8a36" style="-webkit-border-radius: 4px; -moz-border-radius: 4px; border-radius: 4px; color: #ffffff; display: block;">
                        <a href="${generatedLink}" style="font-size: 16px; font-family: Arial, sans-serif; color: #ffffff; text-decoration: none; line-height: 40px; width: 100%; display: inline-block;">View Opportunity</a>
                    </td>
                </tr>
            </table>
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const dataToSend = {
      ...
      Products: opportunityDetails?.map((detail) => ({
        OpportunityProductID: detail?.OpportunityProductID || 0,
        OpportunityID: currentData?.OpportunityID,
        ProductID: detail?.Product?.Product_ID || 0,
        PriceListID: detail?.PriceListID?.PriceListID || 0,
  ...
  const handleAddDetail = () => {
    ...
    if (editingIndex !== null) {
      ...
      updatedDetails[editingIndex] = {
        ...
        OpportunityProductID: updatedDetails[editingIndex]?.OpportunityProductID || 0,
  ...
  const handleEditDetail = (index) => {
    ...
    setValue('CycloUnitPrice', detail?.CycloUnitPrice || null);
    setEditingIndex(index);
  };

  // Table Heads
  const DetailsTableHead =
    approverData[0]?.Approval_Lvl_ID === 2
      ? [
        ...
        <Grid container spacing={3}>
          <Grid xs={12} md={12}>
            <Card sx={{ p: 3 }}>
              ...
              >
                <RHFTextField name="OpportunityName" label="Opportunity Name" fullWidth disabled />
...
OpportunityApprovalForm.propTypes = {
