...
export default function UploadExcelDialog({
  ...
  const InsertVendor = useCallback(
    async (fileData) => {
      ...
      try {
        ...
        //     !tableData.some(
        //       (existingItem) =>
        //         existingItem.Inv_Cat_ID.toLowerCase().trim() ===
        //           item.Inv_Cat_ID.toLowerCase().trim() &&
        //         existingItem.SubCat_Name.toLowerCase().trim() ===
        //           item.SubCat_Name.toLowerCase().trim()
        //     )
        // );
        ...
        const response = await Post(`AddReqmiddleware`, modifiedVendorData);
  ...
  const handleFileChange = async () => {
    ...
    reader.onload = async (m) => {
      ...
      const customHeaders = ['DeptID', 'SectionID', 'InvTypeID', 'CategoryID'];
          ...
          <Grid container>
            <Grid xs={12} md={12}>
              <DialogContent>
                ...
                <a
                  href={`${APP_API}GetDepartmentClassExcel?OrgId=${userData?.userDetails?.orgId}&BranchId=${userData?.userDetails?.branchID}`}
                  download
                >
                  <Button
