...
export default function ApprovalCreateForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const res = await Post('AddDocApprovalDetails', combinedApprovers);
      if (res?.status === 409) {
        ...
        <Grid xs={12} md={12}>
          <Card sx={{ p: 3 }}>
                  ...
                  {selectedAdmins.length > 0 && (
                    <Grid xs={12} md={4}>
                      <Card sx={{ p: 2 }}>
                        ...
                        {selectedAdmins.map((approver) => renderApproverSignature(approver))}
                      </Card>
                    </Grid>
                  )}

                  {selectedKAM.length > 0 && (
                    <Grid xs={12} md={4}>
                      <Card sx={{ p: 2 }}>
                        <Typography variant="subtitle1" gutterBottom>
                          2nd Approvers
                        </Typography>
                        {selectedKAM.map((approver) => renderApproverSignature(approver))}
                      </Card>
                    </Grid>
                  )}
                  ...
                  {selectedGA.length > 0 && (
                    <Grid xs={12} md={4}>
                      <Card sx={{ p: 2 }}>
                        ...
                        {selectedGA.map((approver) => renderApproverSignature(approver))}
                      </Card>
                    </Grid>
                  )}
                </Grid>
            ...
            <QASignDialog
              ...
              title={`Signature for ${currentApprover?.fullName || ''}`}
            /> */}
          </Card>
