...
export default function UploadExcelDialog({
      ...
      try {
        ...
        const filteredData2 = fileData.filter(
          (item) =>
            !tableData.some(
              (existingItem) =>
                existingItem.InvSpecsName.toLowerCase().trim() ===
                item.InvSpecsName.toLowerCase().trim()
            )
        );
        ...
        const response = await Post(`upload/bulkuploadInvItems`, modifiedVendorData);

        if (response.status === 200) {
          enqueueSnackbar('Inventory Items added successfully!', { variant: 'success' });
        }
        uploadClose();
        FetchUpdatedData();
      } catch (error) {
  ...
  const handleFileChange = async () => {
    ...
    reader.onload = async (m) => {
      ...
      const customHeaders = ['InvSpecsName'];
          ...
          <Grid container>
            <Grid xs={12} md={12}>
              <DialogContent>
                ...
                <a href={`${APP_API}export/InvItems`} download>
                  <Button
