        ...
        try {
          ...
          if (response.data.Success && response.data.Header) {
            const formatedData = {
              ...
              Submission_Date: response.data.Header.SubmissionDate
                ? new Date(response.data.Header.SubmissionDate)
                : new Date(),
              Submission_Type: {
              ...
              Bank_Reference_Date: response.data.Header.BankRefDate
                ? new Date(response.data.Header.BankRefDate)
                : new Date(),
              ...
              Negotiation_Date: response.data.Header.NegotiationDate
                ? new Date(response.data.Header.NegotiationDate)
                : new Date(),
              Bank_Maturity_Date: response.data.Header.BankMaturityDate
                ? new Date(response.data.Header.BankMaturityDate)
                : new Date(),
              Bank_Maturity_Received_Date: response.data.Header.BankMaturityReceivedDate
                ? new Date(response.data.Header.BankMaturityReceivedDate)
                : new Date(),

              // Calculate total invoice value from Details
              Total_Invoice_Value:
                response.data.Details?.reduce(
                  (total, item) => total + (parseFloat(item.DocValueUSD) || 0),
                  0
                ) || 0,
      ...
      <CustomBreadcrumbs
        ...
        links={[
          { name: 'Home', href: paths.dashboard.root },
          {
            ...
            href: paths.dashboard.Commercial.export.DocumentSubmission.root,
          },
          { name: 'Edit' },
        ]}
        sx={{
