...
import DocSubmitTableRow from '../DocSubmit-table-row';
import DocSubmitTableToolbar from '../DocSubmit-toolbar';
import DocSubmitTableFiltersResult from '../DocSubmit-filters-result';
...
const TABLE_HEAD = [
  { id: 'SubmissionDate', label: 'Submission Date', minWidth: 140 },
  { id: 'SubmissionTypeName', label: 'Submission Type', minWidth: 140 },
  { id: 'BankName', label: 'Bank Name', minWidth: 140 },
  { id: 'BankRefNo', label: 'Bank Reference No', minWidth: 140 },
  { id: 'BankRefDate', label: 'Bank Reference Date', minWidth: 140 },
  { id: 'TillInvValue', label: 'Till Inv Value', minWidth: 140 },
  { id: 'CurrencyName', label: 'Currency', minWidth: 140 },
  { id: 'CurrencyRate', label: 'Currency Rate', minWidth: 140 },
...
export default function AgencyListView() {
  ...
  const handleDialogOpen = () => {
    navigate(paths.dashboard.Commercial.export.DocumentSubmission.new);
  };
  ...
  const FetchAgent = useCallback(async () => {
    try {
      const response = await Get(
        `CommercialModule/GetExportDocumentSubmissionList?Org_ID=${userData?.userDetails?.orgId}&Branch_ID=${userData?.userDetails?.branchID}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchAgent(), FetchAllCustomers()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchAgent, FetchAllCustomers]);
  ...
  const moveToEditForm = (e) => {
    ...
    navigate(paths.dashboard.Commercial.export.DocumentSubmission.edit(e));
  };
  const moveToPdfView = (e) => {
    navigate(paths.dashboard.Commercial.export.DocumentSubmission.pdf(e));
  };
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.SubmissionTypeName.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        yarn?.BankName.toLowerCase().indexOf(name.toLowerCase()) !== -1 
