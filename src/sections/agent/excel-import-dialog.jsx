...
export default function UploadExcelDialog({
  ...
  const processExcelData = (workbook) => {
    ...
    const combinedData = agentData.map((agent) => {
      const agentContacts = contactsData.filter((contact) => contact.Agent_ID === agent.AgentID);
  ...
  const handleFileChange = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (
          !workbook.SheetNames.includes('AgentMst') ||
          !workbook.SheetNames.includes('Agent_KeyContacts')
        ) {
        ...
        const filteredPayload = payload.filter(
          (item) =>
            !tableData.some(
              (existingItem) =>
                existingItem.Agent_Name.toLowerCase().trim() ===
                item.Agent_Name.toLowerCase().trim()
            )
        );
        ...
        if (response.status === 200) {
          enqueueSnackbar(`${filteredPayload.length} agents added successfully!`, {
            variant: 'success',
          });
          FetchUpdatedData();
          uploadClose();
