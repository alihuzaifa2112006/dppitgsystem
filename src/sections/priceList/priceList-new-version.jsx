...
export default function PriceListEditVersionForm({ currentPriceList, fetch }) {
  ...
  const defaultValues = useMemo(() => {
    ...
    const getNextPriceListVersion = (currentVersion = 'v1.0') => {
    ...
    return {
      ...
      PriceListVer: getNextPriceListVersion(currentPriceList?.Master?.PriceListVer),
      ...
      products:
        currentPriceList?.Details.map((x) => ({
          ...x,
          Color: allColors.find((c) => c.ColorID === x.ColorID) || null,
          Yarn_Type_ID: allTypes.find((y) => y.Yarn_Type_ID === x.Yarn_Type_ID) || null,
          Yarn_Count_ID: allCounts.find((y) => y.Yarn_Count_ID === x.Yarn_Count_ID) || null,
          Composition_ID:
            allCompositions.find((c) => c.Composition_ID === x.Composition_ID) || null,
          Product: null,
          Currency: currencyList.find((c) => c.Currency_ID === x.CurrencyID) || null,
          UOMID: UOM.find((u) => u.UOM_ID === x.UOMID) || null,
          CustomerID: customers.find((c) => c.WIC_ID === x.Cust_ID) || null,
          EndCustomerID: allEndCustomers.find((e) => e.End_Cust_ID === x.End_Cust_ID) || null,
          Year: x?.Year ? new Date(x?.Year) : new Date(),
        })) || [],
    };
  }, [
    currentPriceList,
  ...
  const onSubmit = handleSubmit(async (data) => {
    const mstData = {
      ...
      Prev_Pricelist_ID: data?.Prev_Pricelist_ID,
      Valid_From: data?.Valid_From,
      Valid_Until: data?.Valid_Until,
      PriceListVer: data?.PriceListVer,
      ...
      Details: values?.products
        ...
        ?.map((product) => ({
          ...
          CustomerID: product?.CustomerID?.WIC_ID,
          EndCustomerID: product?.EndCustomerID?.End_Cust_ID,
          Year: product?.Year ? formatDate(product?.Year) : formatDate(new Date()),
    ...
    try {
      const res = await Put(
        `UpdatePriceListNewVersion/${currentPriceList?.Master?.PriceListID}`,
        mstData
      );
      ...
      if (res.status === 200) {
        const newDetails = values?.products
          ?.filter((prd) => !prd.PriceListDtlID)
          .map((product) => ({
            ...product,
            Product_id: product?.Product?.Product_ID || 0,
        ...
        await InsertProductPrice(newDetails);
          ...
          <Grid xs={12} md={12}>
            <Card sx={{ p: 3 }}>
              ...
              >
                ...
                <RHFTextField name="PriceListVer" label="New Version" placeholder="v1.0" disabled />
                <RHFTextField
                      ...
                      <Table>
                        <TableHead>
                          <TableRow>
                            ...
                            <TableCell sx={{ minWidth: 180 }}>Average Price</TableCell>
                            <TableCell align="center" sx={{ minWidth: 280 }}>
                              Price Range
                            </TableCell>
                            {currentPriceList?.Master?.PriceListID !== 2 && (
                              <>
...
PriceListEditVersionForm.propTypes = {
