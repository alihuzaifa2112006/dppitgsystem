...
import FormTableToolbar from '../form-toolbar';
import FormTableFiltersResult from '../form-filters-result';
...
const TABLE_HEAD = [
  { id: 'Name', label: 'Form Name', minWidth: 160 },
  { id: 'Link', label: 'Link', minWidth: 180 },
...
export default function FormListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchForm(), FetchAllRoles()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchForm, FetchAllRoles]);
  ...
  const DeleteDetailTableRow = async (row) => {
    if (AllRoles?.flatMap((roles) => roles?.FormId).includes(row.FormId)) {
      enqueueSnackbar('This form is being used in role database', { variant: 'error' });
      return;
    }
    try {
      await Delete(`DeleteForm/${row.FormId}`);
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Form', href: paths.dashboard.admin.form.root },
              { name: 'list' },
            ]}
            action={
              <Button
                    ...
                    {dataFiltered
                      ...
                      .map((row) => (
                        <FormTableRow
                          key={row?.FormId}
                          row={row}
                          selected={table.selected.includes(row?.FormId)}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) => yarn?.Name.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
      yarn?.Description.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
      yarn?.Link.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
