...
import DocRealizationTableRow from '../DocRealization-table-row';
import DocRealizationTableToolbar from '../DocRealization-toolbar';
import DocRealizationTableFiltersResult from '../DocRealization-filters-result';
import { alpha, Tab, Tabs } from '@mui/material';
import Label from 'src/components/label';

import { Box } from '@mui/system';

import ReportDialog from 'src/sections/agency/ReportDialog';
import UploadExcelDialog from '../excel-import-dialog';

// ----------------------------------------------------------------------
const TABLE_HEAD = [
  { id: 'DCRealizationNo', label: 'DC Realization No', minWidth: 160 },
  { id: 'CurrencyName', label: 'Currency', minWidth: 140 },
  { id: 'DDRate', label: 'OD Rate', minWidth: 90 ,align:'right'},
  { id: 'TotalInvoiceValue', label: 'Total Invoice Value', minWidth: 120 ,align:'right'},
  { id: 'FinalPaymentRecvDate', label: 'Final Payment Receive Date', minWidth: 180 },
  { id: 'SubmissionBankName', label: 'Submission Bank', minWidth: 140 },
  { id: 'PRCNo', label: 'PRC No', minWidth: 90 },
...
export default function DocRealizationListView() {
  ...
  const handleDialogOpen = () => {
    navigate(paths.dashboard.Commercial.export.DocumentRealization.new);
  };
...
const FetchBankRealization = useCallback(async () => {
  try {
    const response = await Get(
      `CommercialModule/GetExportLCRealizations?Org_ID=${userData?.userDetails?.orgId}&Branch_ID=${userData?.userDetails?.branchID}`
    );
    
    // Format dates and add any additional processing if needed
    

    setTableData(response.data.Data);
  } catch (error) {
    console.log('Error fetching bank realizations:', error);
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchBankRealization(), FetchAllCustomers()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchBankRealization, FetchAllCustomers]);
  ...
  const moveToEditForm = (e) => {
    ...
    navigate(paths.dashboard.Commercial.export.DocumentRealization.edit(e));
  };
  const moveToPdfView = (e) => {
    navigate(paths.dashboard.Commercial.export.DocumentRealization.pdf(e));
  };
      ...
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          ...
          <Card>
            ...
            {canReset && (
              <DocRealizationTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
                //
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.DCRealizationNo.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        yarn?.SubmissionBankName.toLowerCase().indexOf(name.toLowerCase()) !== -1 
