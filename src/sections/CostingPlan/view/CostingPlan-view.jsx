...
import CostingPlanTableToolbar from '../CostingPlan-toolbar';
import CostingPlanTableFiltersResult from '../CostingPlan-filters-result';
...
export default function CostingPlanListView() {
 ...
 const flattenRequestData = (requests) => requests.flatMap(request => 
  ...
  const FetchCostingPlanData = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchCostingPlanData()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchCostingPlanData]);
  ...
  const moveToEditForm = (e) => {
    // navigate(paths.dashboard.AIPlans.CostingPlan.edit(e));
  };

  const DeleteDetailTableRow = async (id) => {
    try {
      await Delete(`CostingPlan/delete/${id}/${userData?.userDetails?.userId}`);
