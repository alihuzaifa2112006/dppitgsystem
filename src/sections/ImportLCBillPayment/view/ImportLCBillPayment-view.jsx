...
import ImportLCBillPaymentTableRow from '../ImportLCBillPayment-table-row';
import ImportLCBillPaymentTableToolbar from '../ImportLCBillPayment-toolbar';
import ImportLCBillPaymentTableFiltersResult from '../ImportLCBillPayment-filters-result';
import axios from 'axios';

const TABLE_HEAD = [
  { id: 'ImportLCBillPaymentNo', label: 'Import LC Bill Payment No.', minWidth: 145 },
...
export default function ImportLCBillPaymentListView() {
  ...
  const FetchImportLCBillPaymentData = useCallback(async () => {
    try {
      const response = await Get(
        `GetImportLCBillPaymentActiveinActiveList?OrgID=${userData?.userDetails?.orgId}&BranchID=${userData?.userDetails?.branchID}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchImportLCBillPaymentData()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchImportLCBillPaymentData]);
  ...
  const moveToEditForm = (e) => {
    navigate(paths.dashboard.Commercial.import.ImportLCBillPayment.edit(e));
  };
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Import LC Bill Payment', href: paths.dashboard.Commercial.import.ImportLCBillPayment.root },
              { name: 'List' },
            ]}
            action={
              <Button
          ...
          <Card>
            <ImportLCBillPaymentTableToolbar
              filters={filters}
            ...
            {canReset && (
              <ImportLCBillPaymentTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
                //
                ...
                >
                  ...
                  <TableBody>
                    {dataFiltered
                      ...
                      .map((row) => (
                        <ImportLCBillPaymentTableRow
                          key={row?.ImportInvoiceEntryID}
                          row={row}
                          selected={table.selected.includes(row?.ImportLCBillPaymentID)}
                          onEditRow={() => moveToEditForm(row?.ImportLCBillPaymentID)}
                        />
                      ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.ImportLCBillPaymentNo.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
