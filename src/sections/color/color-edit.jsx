...
import CompositionDialog from 'src/components/composition-diaglog';
import AutocompleteWithAdd from 'src/components/AutocompleteWithAdd';
import Scrollbar from 'src/components/scrollbar';

// ----------------------------------------------------------------------

export default function ColorEditForm({ currentData, currentDetails }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      isMultiColor: currentData?.SL === '1' || currentData?.SL === 1,
      ColorName: currentData?.ColorName || '',
      ColorFamily:
        allColorFamily.find((item) => item.ColorFamilyID === currentData?.ColorFamilyID) || '',
      ColorNickName: currentData?.Color_AND_Code,
      Color_Code: currentData?.Color_Code || '',
      HexCode: currentData?.HexCode || '',
      ShadeType: shadeTypes.find((item) => item.TypeID === currentData?.TypeID) || null,
      Customer: customers.find((item) => item.WIC_ID === currentData?.CustomerID) || null,
      DataColorAndCode: currentData?.DataColorAndCode || '',
      colorMap: currentDetails.map((detail) => ({
        ...detail,
        ColorDtlID: detail.ColorDtlID,
        ColorReference: allColors.find((x) => x.ColorID === detail.ColorRefID) || null,
        DataColorAndCode: detail?.DataColorAndCode || '',
      })),
    }),
    [currentData, currentDetails, allColors, allColorFamily, shadeTypes, customers]
  );
  ...
  useEffect(() => {
    if (!isLoading && currentData) {
      // Ensure isMultiColor is computed correctly
      const isMulti = Boolean(currentData?.SL === '1' || currentData?.SL === 1);
      const resetValues = {
        ...defaultValues,
  ...
  // Populate DataColorAndCode in colorMap when ColorReference changes
  useEffect(() => {
    if (values?.isMultiColor && values?.colorMap) {
      const updatedColorMap = values.colorMap.map((item) => {
        // Always sync DataColorAndCode with ColorReference if ColorReference exists
        if (item?.ColorReference?.DataColorAndCode) {
          return {
            ...item,
            DataColorAndCode: item.ColorReference.DataColorAndCode,
          };
        }
        // If no ColorReference, keep existing DataColorAndCode or empty string
        return {
          ...item,
          DataColorAndCode: item.DataColorAndCode || '',
        };
      });
      
      // Check if any item needs updating
      const needsUpdate = updatedColorMap.some((item, index) => {
        const currentItem = values.colorMap[index];
        return item.DataColorAndCode !== (currentItem?.DataColorAndCode || '');
      });
      
      if (needsUpdate) {
        setValue('colorMap', updatedColorMap, { shouldValidate: false });
      }
    }
    // eslint-disable-next-line
  }, [
    ...
    // eslint-disable-next-line
    ...(values.colorMap || []).flatMap((color, index) => [
      values.colorMap?.[index]?.ColorReference?.ColorID,
      values.colorMap?.[index]?.ColorReference?.DataColorAndCode,
    ]),
    setValue,
  ]);
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const existingColor = tableData.find(
      (color) =>
        color.ColorID !== currentData?.ColorID &&
        color.ColorNameandCode.trim().toLowerCase() === data?.ColorNickName?.trim().toLowerCase()
    ...
    const dataToSend = {
      ...
      Color_AND_Code: data.ColorNickName,
      ...
      Details: data?.isMultiColor
        ? data?.colorMap.map((detail) => ({
            ...
            DataColorAndCode: detail.DataColorAndCode?.trim() || detail.ColorReference?.DataColorAndCode?.trim() || '',
    ...
    try {
      await Put(`color/update`, dataToSend).then(async (res) => {
