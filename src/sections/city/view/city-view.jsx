...
import CityTableToolbar from '../city-toolbar';
import CityTableFiltersResult from '../city-filters-result';
import { alpha, Tab, Tabs } from '@mui/material';
import Label from 'src/components/label';
import CityDialog from '../AddDialog';
import CityEditDialog from '../EditDialog';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'City_Name', label: 'City', minWidth: 160 },
...
export default function CityListView() {
  ...
  const FetchCity = useCallback(async () => {
    try {
      const response = await Get(`GetAllcity`);
      const decryptedData = response.data?.Data.map((item) => ({
        ...item,
        Country_Name: item?.Country_Name || '-',
        IsActive: item?.is_Active === true ? 'Active' : 'Inactive',
      }));
  ...
  return (
    <>
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'City', href: paths.dashboard.admin.room.root },
              { name: 'list' },
            ]}
            action={
              <Button
                    ...
                    {dataFiltered
                      ...
                      .map((row) => (
                        <CityTableRow
                          key={row?.City_ID}
                          row={row}
                          selected={table.selected.includes(row?.City_ID)}
      ...
      <CityDialog uploadClose={handleDialogClose} uploadOpen={dialogOpen} tableData={tableData} />
      <CityEditDialog
