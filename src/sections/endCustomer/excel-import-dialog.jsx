...
export default function UploadEndCustomerExcelDialog({
  ...
  const processExcelData = (workbook) => {
    ...
    const contactsHeaders = ['ID', 'Contact_Name', 'Contact_Number', 'Email_Address'];
    ...
    const combinedData = customerData.map((customer) => {
      const customerContacts = contactsData.filter((contact) => contact.ID === customer.ID);
  ...
  const handleFileChange = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (
          !workbook.SheetNames.includes('End_Customer') ||
          !workbook.SheetNames.includes('End_Customer_KeyContacts')
        ) {
          throw new Error(
            'Excel file must contain both End_Customer and End_Customer_KeyContacts sheets'
          );
        }
        ...
        const filteredPayload = payload.filter(
          (item) =>
            !tableData.some(
              (existingItem) =>
                existingItem.End_Cust_Name_Org.toLowerCase().trim() ===
                item.End_Cust_Name.toLowerCase().trim()
            )
        );

        if (filteredPayload.length === 0) {
          enqueueSnackbar('These end customers already exist', { variant: 'info' });
          return;
        }
