...
import RoomTableToolbar from '../room-toolbar';
import RoomTableFiltersResult from '../room-filters-result';
...
const TABLE_HEAD = [
  { id: 'Storage_name', label: 'Storage Location', minWidth: 160 },
  { id: 'Unit_Location_name', label: 'Unit Location', minWidth: 140 },
...
export default function RoomListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchRoom(), FetchAllRoles(), FetchAllSections()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchRoom, FetchAllRoles, FetchAllSections]);
  ...
  const uniqueCountry_Name = [...new Set(tableData.map((tb) => tb?.Unit_Location_name))];
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Storage Location', href: paths.dashboard.admin.room.root },
              { name: 'list' },
            ]}
            action={
              <Button
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.Storage_name.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        yarn?.Unit_Location_name.toLowerCase().indexOf(name.toLowerCase()) !== -1
  ...
  if (role.length) {
    inputData = inputData.filter((yarn) => role.includes(yarn?.Unit_Location_name));
  }
