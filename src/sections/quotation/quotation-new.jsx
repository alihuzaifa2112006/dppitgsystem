...
export default function QuotationCreateForm() {
  ...
  const TotalAmount = quotationDetails.reduce((total, detail) => {
    ...
    const currencyID = detail?.PriceListID?.CurrencyID || 1;
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const dataToSend = {
      OpportunityID: data.Opportunity?.OpportunityID || 0,
      Doc_App_Setup_ID: 1,
      KAM: data?.KAM?.UserID,
      Currency_ID: 1,
  ...
  const handleAddDetail = () => {
    ...
    if (editingIndex !== null) {
      ...
      updatedDetails[editingIndex] = {
        Description: values?.Remarks || 'N/A',
        Product: values.Product,
        Remarks: values?.Description || generateProductName() || 'N/A',
  ...
  const handleEditDetail = (index) => {
    ...
    setValue('Remarks', detail.Description);
    // setValue('Description', detail.Remarks);
                    ...
                    >
                      ...
                      >
                        <RHFAutocomplete
                          ...
                          key={values.PriceListID?.Product_ID || 'new'} // changes when reset
                          name="PriceListID"
                          label="Active Pricelist"
                          placeholder="Choose an option"
                          fullWidth
                          options={allPriceList}
                          value={values.PriceListID || null}
                          getOptionLabel={(option) => option?.PriceListName || ''}
                          isOptionEqualToValue={(option, value) => {
                            if (!option || !value) return false;
                            return option.PriceListID === value.PriceListID;
                          }}
                        />
                        <Box sx={{ display: 'flex', justifyContent: 'end', alignItems: 'center' }}>
                          <Button
                            ...
                            color="primary"
                            size="small"
                            disabled={!values.PriceListID?.PriceListID}
