...
import FabricTableRow from '../fabric-table-row';
import FabricTableToolbar from '../fabrictoolbar';
import FabricTableFiltersResult from '../fabric-filters-result';
import { alpha, Tab, Tabs } from '@mui/material';
import Label from 'src/components/label';
import FabricDialog from '../AddDialog';
import FabricEditDialog from '../EditDialog';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'Fabric_Types', label: 'Fabric Type', minWidth: 160 },
  // { id: 'Fabric_Name', label: 'Fabric', minWidth: 140 },
...
export default function FabricListView() {
  ...
  const FetchFabric = useCallback(async () => {
    try {
      const response = await Get(`GetFabricTypes?OrgID=${userData?.userDetails?.orgId}&BranchID=${userData?.userDetails?.branchID}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchFabric(), FetchCity()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchFabric, FetchCity]);
  ...
  const uniqueFabric_Name = [...new Set(tableData.map((tb) => tb.Fabric_Types))];
  ...
  const moveToEditForm = (e) => {
    if (allCities.find((city) => city.Fabric_ID === e.Fabric_ID)) {
      enqueueSnackbar('This fabric is in use!', { variant: 'error' });
      return;
    }
  ...
  const DeleteDetailTableRow = async (row) => {
    ...
    // if (allCities.some((city) => city.Fabric_ID === row.Fabric_ID)){
    //   enqueueSnackbar('This fabric is in use!', { variant: 'error' });
    //   return;
    // }
    try {
      await Delete(`DeleteFabricType?FabricTypeID=${row.Fabric_TypeID}`);
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Fabric', href: paths.dashboard.powertools.fabric },
              { name: 'list' },
            ]}
            action={
              <Button
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.Fabric_Types.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
  ...
  if (role.length) {
    inputData = inputData.filter((yarn) => role.includes(yarn?.Fabric_Types));
  }
