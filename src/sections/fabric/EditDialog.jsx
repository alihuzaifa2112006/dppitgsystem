...
export default function FabricEditDialog({ uploadClose, uploadOpen, row, tableData }) {
  ...
  const NewFabricSchema = Yup.object().shape({
    Fabric_Name: Yup.string()
      ...
      .min(3, 'Fabric Name must be at least 3 characters long')
      .max(100, 'Fabric Name must be less than or equal to 100 characters'),
    // .matches(/^[a-zA-Z\s]+$/, 'Fabric Name must only contain letters and spaces'),
  });
  ...
  const PutFabricData = async (PutData) => {
    try {
      await Put(`fabric/${row?.Fabric_ID}`, PutData).then(async (res) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (
      tableData.some(
        (item) => item.Fabric_Name === data.Fabric_Name && row.Fabric_Name !== data.Fabric_Name
      )
    ) {
      enqueueSnackbar('Fabric already exists', { variant: 'error' });
      return;
    }
    try {
      const dataToSend = {
      ...
      await PutFabricData(dataToSend);
      uploadClose();
    } catch (error) {
      console.error(error);
    }
      ...
      >
        ...
        <DialogContent sx={{ py: 3 }}>
          <FormProvider methods={methods} onSubmit={onSubmit}>
            ...
            >
              <RHFTextField name="Fabric_Name" label="Fabric Name" />
            </Box>
            <Box
              display="flex"
              alignItems="center"
