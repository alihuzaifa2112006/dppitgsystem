...
export default function AssignApprovalForm({ currentData }) {
  ...
  const [AssignDetails, setAssignDetails] = useState(currentData?.OppProduct || []);
      ...
      try {
        ...
        if (
          response.data.Data?.find(
            (product) =>
              product.Product_ID === AssignDetails[editingIndex]?.Product?.Product_ID
          )
        ) {
          setSelectedProduct(
            response.data.Data?.find(
  ...
  const TotalAmount = AssignDetails.reduce((total, detail) => {
  ...
  const defaultValues = useMemo(
    () => ({
      AssignName: currentData?.AssignName || '',
      ...
      AssignDate: currentData?.AssignDate || null,
  ...
  const PostAssignMasterData = async (opData) => {
    try {
      await Put('UpdateAssignWithProducts', opData).then(async (res) => {
        if (res.status === 200) {
          // enqueueSnackbar('Created Successfully!');
          // reset();
          // router.push(paths.dashboard.transaction.Assign.root);
        }
      });
    } catch (error) {
      console.log(error);
  ...
  const SendApproval = async (yesORno) => {
    ...
    const dataToSend = {
      Assign: {
        AssignID: currentData?.AssignID,
        AssignName: values.AssignName,
      ...
      Products: AssignDetails
        ?.filter((detail) => detail?.updated)
        ?.map((detail) => ({
          AssignProductID: detail?.AssignProductID || 0,
          AssignID: currentData?.AssignID,
    ...
    try {
      await PostAssignMasterData(dataToSend);

      const respones = await Post(`UpdateAssignApproval`, {
        AssignID: currentData?.AssignID,
      ...
      if (respones?.status === 200) {
        enqueueSnackbar('Assign Approved Successfully', { variant: 'success' });
        const generatedLink = `${APP_URL}${paths.dashboard.transaction.Assign.approval(
          currentData?.AssignID
        )}`;
        const emailData = {
          AssignID: currentData?.AssignName,
          EmailTo: 'hasham25525@gmail.com', // or currentData?.EmailAddress
          Subject: 'Assign Approved',
          Body: `<!DOCTYPE html>
    <html>
    <head>
        <style>
    ...
    <body>
        <div class="container">
            <h2 class="header">Assign Approved</h2>

            <p>Dear ${currentData?.KAM_Name},</p>

            <p>The following Assign has been approved and is ready for processing:</p>

            <div class="details">
                <p><strong>Assign Name:</strong> ${currentData?.AssignName}</p>
                <p><strong>Approved By:</strong> ${userData?.userDetails?.userName}</p>
                <p><strong>Approval Date:</strong> ${fDate(new Date())}</p>

            </div>

            <p>You can view the details of the approved Assign below:</p>
            ...
            <table cellspacing="0" cellpadding="0" style="margin-bottom: 10px;">
                <tr>
                    <td align="center" width="200" height="40" bgcolor="#5e8a36" style="-webkit-border-radius: 4px; -moz-border-radius: 4px; border-radius: 4px; color: #ffffff; display: block;">
                        <a href="${generatedLink}" style="font-size: 16px; font-family: Arial, sans-serif; color: #ffffff; text-decoration: none; line-height: 40px; width: 100%; display: inline-block;">View Assign</a>
                    </td>
                </tr>
            </table>
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (AssignDetails.length === 0) {
      enqueueSnackbar('Please add at least one Assign product', { variant: 'error' });
      return;
    }
    ...
    const dataToSend = {
      ...
      Products: AssignDetails?.map((detail) => ({
        AssignProductID: detail?.AssignProductID || 0,
        AssignID: currentData?.AssignID,
        ProductID: detail?.Product?.Product_ID || 0,
        PriceListID: detail?.PriceListID?.PriceListID || 0,
  ...
  const availableProducts = useMemo(() => {
    const addedProductIds = AssignDetails.map((detail) => detail.Product?.Product_ID);
    return allProducts.filter((product) => !addedProductIds.includes(product.Product_ID));
  }, [AssignDetails, allProducts]);
  ...
  const handleAddDetail = () => {
    ...
    if (editingIndex !== null) {
      ...
      updatedDetails[editingIndex] = {
        ...
        AssignProductID: updatedDetails[editingIndex]?.AssignProductID || 0,
        ...
        <Grid container spacing={3}>
          <Grid xs={12} md={12}>
            <Card sx={{ p: 3 }}>
              ...
              >
                <RHFTextField name="AssignName" label="Assign Name" fullWidth disabled />
