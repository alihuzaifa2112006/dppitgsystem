...
import AssignEditForm from '../assign-edit';
import { useEffect, useState } from 'react';
import { Get } from 'src/api/apibasemethods';

// ----------------------------------------------------------------------

export default function AssignEditView({ urlData }) {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await Get(`GetAssignById/${urlData?.AssignID}`);
      const formatedData = {
        ...response.data,
        AssignDate: new Date(response.data.AssignDate),
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      ...
      {currentData && <AssignEditForm currentData={currentData} />}
    </Container>
  );
}

AssignEditView.propTypes = {
