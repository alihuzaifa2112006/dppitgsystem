...
export default function CountryEditDialog({ uploadClose, uploadOpen, row, tableData }) {
  ...
  const NewCountrySchema = Yup.object().shape({
    PurposeName: Yup.string()
      ...
      .min(3, 'Purpose Name must be at least 3 characters long')
      .max(100, 'Purpose Name must be less than or equal to 100 characters'),
  });
  ...
  const PutPurposeData = async (putData) => {
    try {
      const res = await Put(`UpdatePurpose`, putData);
  ...
  const onSubmit = handleSubmit(async (data) => {
    if (
      tableData.some(
        (item) => item.Purposes === data.PurposeName && row?.Purposes !== data.PurposeName
      )
    ) {
    ...
    const dataToSend = {
      // PurposeID: row?.PurposeID || row?.PurposeID,
      PurposeID: row?.PurposeID,
      // Purposes: data.PurposeName,
      Purposes: data.PurposeName.trim(),
    ...
    await PutPurposeData(dataToSend);
  });
      ...
      >
        ...
        <DialogContent sx={{ py: 3 }}>
          <FormProvider methods={methods} onSubmit={onSubmit}>
            ...
            >
              <RHFTextField name="PurposeName" label="Purpose Name" />
            </Box>
