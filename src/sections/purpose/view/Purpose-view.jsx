...
import PurposeTableRow from '../Purpose-table-row';
import PurposeTableToolbar from '../Purposetoolbar';
import PurposeTableFiltersResult from '../Purpose-filters-result';
import { alpha, Tab, Tabs } from '@mui/material';
import Label from 'src/components/label';
import PurposeDialog from '../AddDialog';
import PurposeEditDialog from '../EditDialog';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'Purpose_Name', label: 'Purpose Name', minWidth: 160 },
...
export default function PurposeListView() {
  ...
  //   }
  // }, []);
  // console.log(Purpose,"thisis console")

  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchPurpose(),]);
      setLoading(false);
    };
    fetchData();
  }, [FetchPurpose ]);
...
const uniquePurpose_Name = [...new Set(tableData.map((tb) => tb.Purpose_Name))];
  ...
  const moveToEditForm = (e) => {
    if (Purpose.find((city) => city.Purpose_ID === e.Purpose_ID)) {
      enqueueSnackbar('This Purpose is in use!', { variant: 'error' });
      return;
    }
 ...
 const DeleteDetailTableRow = async (row) => {
  ...
  if (Purpose.some((city) => city.Purpose_ID === row.Purpose_ID)) {
    enqueueSnackbar('This Purpose is in use!', { variant: 'error' });
    return;
  }
  ...
  try {
    const res = await Put('DeletePurpose', dataToSend); // ✅ PUT instead of DELETE
    enqueueSnackbar(res?.data?.Message || 'Deleted successfully', { variant: 'success' });
    FetchPurpose(); // Refetch updated purpose list
  } catch (error) {
    ...
    <>
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            heading="Purpose List"
            links={[
              { name: 'Home', href: paths.dashboard.root },
              { name: 'Purpose', href: paths.dashboard.powertools.Purpose },
              { name: 'list' },
            ]}
            action={
              <Button
          ...
          <Card>
            ...
            <PurposeTableToolbar
              ...
              roleOptions={uniquePurpose_Name}
                    ...
                    {dataFiltered
                      ...
                      .map((row) => (
                        <PurposeTableRow
                          key={row?.PurposeID}
                          row={row}
                          selected={table.selected.includes(row?.PurposeID)}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.Purpose_Name.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
  ...
  if (role.length) {
    inputData = inputData.filter((yarn) => role.includes(yarn?.Purpose_Code));
  }
