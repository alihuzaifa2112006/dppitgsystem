...
import ProductTableRow from '../raw-table-row';
import ProductTableToolbar from '../raw-toolbar';
import ProductTableFiltersResult from '../raw-filters-result';
...
const TABLE_HEAD = [
  { id: 'RM_Code', label: 'RM Code', minWidth: 160 },
  // { id: 'Commercial_Name', label: 'Product Name', minWidth: 140 },
  { id: 'Blend_Name', label: 'Raw Material', minWidth: 140 },
  { id: 'ColorName', label: 'Color Name', minWidth: 140 },
  // { id: 'RM_Dimension', label: 'Dimension', minWidth: 140 },
  { id: 'RM_OpeningStock', label: 'Quantity', minWidth: 120, align: 'right' },
  ...
  const FetchProductData = useCallback(async () => {
    try {
      const response = await Get(
        `GetRawMaterialssource?orgId=${userData?.userDetails?.orgId}&branchId=${userData?.userDetails?.branchID}`
      );
    ...
    <>
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            heading="Raw Material Description"
            links={[
              { name: 'Home', href: paths.dashboard.root },
              {
                name: 'Raw Material Description',
                href: paths.dashboard.InventoryManagement.rawMaterial.root,
              },
              { name: 'List' },
            ]}
            action={
              ...
              >
                ...
                <Button
                  ...
                  color="primary"
                  onClick={() => {
                    router.push(paths.dashboard.InventoryManagement.rawMaterial.new);
                  }}
                >
                  Add Raw Material
                </Button>
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.RM_Code.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        yarn?.Blend_Name.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
