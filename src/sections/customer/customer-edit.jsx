...
export default function CustomerEditForm({ urlData }) {
  ...
  const GetCustomerByID = useCallback(async () => {
    try {
      const response = await Get(
        `GetCustomerByID?CustomerMasterID=${urlData.customerID}`
      );
      const decryptedData = decryptObjectKeys(response.data.ServiceRes);
  ...
  const UpdateDetailData = async (detail) => {
    try {
      ...
      await Put(`UpdateCustomerDetail`, encryptedData);
    } catch (error) {
      console.log('Detail', error);
    }
  };

  const UpdateCustomerMasterData = async () => {
    ...
    const UpdateMasterData = {
      CustomerMasterID: customerModel.CustomerMasterID,
      ContractDate: customerModel.ContractDate,
      SupplierID: customerModel.SupplierID,
      PaymentTermID: customerModel.PaymentTermID,
    ...
    try {
      ...
      await Put('UpdateCustomerMaster', encryptedData).then(async (res) => {
        // if (res.data.ResponseCode === '100') {
        const UpdateDetails = customerDetails
          .filter((detail) => detail?.CustomerDetailID) // Filter details with CustomerDetailID
          .map((detail) => ({
        ...
        const InsertDetails = customerDetails
          .filter((detail) => !detail?.CustomerDetailID) // Filter details without CustomerDetailID
          .map((detail) => ({
            CustomerMasterID: customerModel?.CustomerMasterID, // Default or existing MasterID
            YarnDatabaseID: detail?.YarnDatabaseID,
            UnitID: detail?.UnitID,
            Quantity: detail?.Quantity,
            UnitPrice: detail?.UnitPrice,
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      await UpdateCustomerMasterData();
    } catch (error) {
      console.error(error);
    }
  });
  ...
  const handleAddDetail = () => {
    ...
    // Check if CustomerDetailID exists
    if (yarnDetail.CustomerDetailID) {
      // Update the existing detail
      setCustomerDetails((prevDetails) =>
        prevDetails.map((detail) =>
          detail.CustomerDetailID === yarnDetail.CustomerDetailID
      ...
      // Add a new detail
      setCustomerDetails((prevDetails) => [...prevDetails, yarnDetail]);
  ...
  const DeleteDetailTableRow = async (rowToDelete) => {
    ...
    try {
      await Delete(
        `DeleteContractDetail?CustomerDetailID=${rowToDelete?.CustomerDetailID}`
      );
            ...
            <Card sx={{ p: 3 }}>
              ...
              >
                <RHFTextField
                  ...
                  value={customerModel?.ContractNo}
                  name="ContractNo"
                  label="Contract Number"
                  // onchange={(e) =>
                  //   setCustomerModel({ ...customerModel, ContractNo: e.target.value })
                  // }
                />
                ...
                <RHFAutocomplete
                  ...
                  value={suppliers?.find(
                    (option) => option?.SupplierDatabaseID === customerModel?.SupplierID
                  )}
                  options={suppliers}
                ...
                <RHFAutocomplete
                  ...
                  value={paymentTerms?.find(
                    (option) => option?.PaymentTermID === customerModel?.PaymentTermID
                  )}
