...
import LocationTableToolbar from '../location-toolbar';
import LocationTableFiltersResult from '../location-filters-result';
...
const TABLE_HEAD = [
  { id: 'StoreName', label: 'Unit Location', minWidth: 160 },
 ...
 const FetchYarnTypeData = useCallback(async () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchLocation(), FetchAllRoles(), FetchAllSections()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchLocation, FetchAllRoles, FetchAllSections]);
  ...
  const DeleteDetailTableRow = async (row) => {
    if (AllRoles?.some((role) => role.DPT_ID === row.Dpt_ID)) {
      enqueueSnackbar('This location is being used in role database', { variant: 'error' });
      return;
    }
    if (AllSections?.some((role) => role.DPT_ID === row.Dpt_ID)) {
      enqueueSnackbar('This location is being used in section database', { variant: 'error' });
      return;
    }
    try {
    ...
    <>
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            heading="Unit Location"
            links={[
              { name: 'Home', href: paths.dashboard.root },
              { name: 'Unit Location', href: paths.dashboard.admin.location.root },
              { name: 'list' },
            ]}
            action={
              <Box
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) => yarn?.StoreName.toLowerCase().indexOf(name.toLowerCase()) !== -1
