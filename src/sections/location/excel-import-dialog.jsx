...
export default function UploadExcelDialog({
      ...
      try {
        ...
        const filteredData2 = fileData.filter(
          (item) =>
            !tableData.some(
              (existingItem) =>
                existingItem.Unit_Location_name.toLowerCase().trim() ===
                item.Unit_Location_name.toLowerCase().trim()
            )
        );
        ...
        const response = await Post(`UnitLocationbulkupload`, modifiedVendorData);
  ...
  const handleFileChange = async () => {
    ...
    reader.onload = async (m) => {
      ...
      const customHeaders = ['Unit_Location_name'];
          ...
          <Grid container>
            <Grid xs={12} md={12}>
              <DialogContent>
                ...
                <a href="https://apicyclo.scmcloud.online/Uploads/UnitLocationBulkUpload.xlsx" download>
                  <Button
