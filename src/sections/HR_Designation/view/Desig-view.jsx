...
import DesignationTableRow from '../Desig-table-row';
import DesignationTableToolbar from '../Desig-toolbar';
import DesignationTableFiltersResult from '../Desig-filters-result';
import { alpha, Tab, Tabs } from '@mui/material';
import Label from 'src/components/label';
import DesignationDialog from '../AddDialog';
import DesignationEditDialog from '../EditDialog';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'DesignationName', label: 'Designation Name', minWidth: 160 },
...
export default function DesignationListView() {
  ...
  const FetchDesignation = useCallback(async () => {
    try {
      const response = await Get(
        `HRModule/GetDesignationList?Org_ID=${userData?.userDetails?.orgId}&Branch_ID=${userData?.userDetails?.branchID}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchDesignation(), FetchCity()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchDesignation, FetchCity]);
  ...
  const moveToEditForm = (e) => {
    if (allCities.find((city) => city.Country_ID === e.Country_ID)) {
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Designation', href: paths.dashboard.HR_Module.Setup.designation },
              { name: 'list' },
            ]}
            action={
              <Button
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.DesignationName.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        yarn?.RoleName.toLowerCase().indexOf(name.toLowerCase()) !== -1
