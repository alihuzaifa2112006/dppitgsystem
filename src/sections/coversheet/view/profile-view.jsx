...
const TABLE_HEAD = [
  ...
  { id: 'isMature', label: 'Status', align: 'center', minWidth: 140 },
  // { id: 'UnitName', label: 'Unit Name', align: 'center', minWidth: 140 },
  // { id: 'CurrencyName', label: 'CurrencyName' },
  // { id: 'Quantity', label: 'Quantity' },

  { id: '', label: 'Score Card', width: 128, align: 'center' },
];
...
export default function ProfileListView() {
  ...
  const moveToEditForm = (e) => {
    navigate(paths.dashboard.customer.coversheet.view(e));
  };
              ...
              {STATUS_OPTIONS.map((tab) => (
                <Tab
                  ...
                  icon={
                    ...
                    >
                      {['Y', 'N', 'R'].includes(tab.value)
                        ? tableData.filter((td) => td.isMature === tab.value).length
                        : tableData.length}
                    </Label>
                  }
                />
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        ...
        yarn?.isMature.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((supplier) => supplier.isMature === status);
  }
