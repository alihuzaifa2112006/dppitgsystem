...
export default function ProfileTableRow({ row, selected, onEditRow, getStatusColor }) {
  const { Cust_Name, City_Name, Country_Name, Cust_Bus_type, BranchName } = row;
  ...
  return (
    <TableRow hover selected={selected}>
      ...
      <TableCell sx={{ whiteSpace: 'nowrap', textAlign: 'center' }}>
        <Label color={getStatusColor(row.isMature)}>
          {/* eslint-disable-next-line */}
          {row.isMature === 'Y' ? 'Approved' : row.isMature === 'N' ? 'New' : 'Rejected'}
        </Label>
      </TableCell>
      {/* <TableCell sx={{ whiteSpace: 'nowrap', textAlign: 'center' }}>
