...
export default function UploadExcelDialog({
  ...
  const InsertVendor = useCallback(
    async (fileData) => {
      ...
      try {
        // check if the Yarn_Count_Name and Yarn_Code exists in the table, if yes then exclude them from the fileData
        const filteredData2 = fileData.filter(
          (item) =>
            !tableData.some(
              (existingItem) =>
                existingItem.Yarn_Count_Name.toLowerCase().trim() ===
                item.Yarn_Count_Name.toLowerCase().trim()
            )
        );
        ...
        const response = await Post(`yarncountbulkupload`, modifiedVendorData);

        if (response.status === 200) {
          enqueueSnackbar('Yarn Counts added successfully!', { variant: 'success' });
        }
        uploadClose();
        FetchUpdatedData();
      } catch (error) {
  ...
  const handleFileChange = async () => {
    ...
    reader.onload = async (m) => {
      ...
      const customHeaders = ['Yarn_Count_Name'];
          ...
          <Grid container>
            <Grid xs={12} md={12}>
              <DialogContent>
                ...
                <a
                  href="https://apicyclo.scmcloud.online/Uploads/YarnCountBulkUpload.xlsx"
                  download
                >
                  <Button
