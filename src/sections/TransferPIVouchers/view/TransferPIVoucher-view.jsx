...
import TransferPIVoucherTableRow from '../TransferPIVoucher-table-row';
import TransferPIVoucherTableToolbar from '../TransferPIVoucher-toolbar';
import TransferPIVoucherTableFiltersResult from '../TransferPIVoucher-filters-result';
import axios from 'axios';
import { Box } from '@mui/system';
// import UploadExcelDialog from '../TransferPIVouchers-excel-import-dialog';
...
export default function TransferPIVoucherListView() {
 ...
 const flattenRequestData = (requests) => requests.flatMap(request => 
  ...
  const FetchTransferPIData = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchTransferPIData()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchTransferPIData]);
  ...
  const moveToEditForm = (e) => {
    // navigate(paths.dashboard.Production.ProductRequest.edit(e));
  };

  const DeleteDetailTableRow = async (id) => {
    try {
      await Delete(`ProductRequest/delete/${id}/${userData?.userDetails?.userId}`);
      ...
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
            ...
            action={
              ...
              >
                <Button
                  ...
                  href={paths.dashboard.Production.ProductVoucher.new}
                  variant="contained"
          ...
          <Card>
            ...
            {canReset && (
              <TransferPIVoucherTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
