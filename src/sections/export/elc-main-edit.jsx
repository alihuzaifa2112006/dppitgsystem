...
export default function ExportEditFormMain({ currentData }) {
  ...
  const defaultValues = useMemo(() => ({
    ...
    Consignee: consignParties?.find((option) => option?.ConsigneeID === currentData?.ConsigneeID) || null,
    NotifyParty: allNotifyParties?.find((option) => option?.NotifyPartyID === currentData?.NotifyPartyID) || null,
  ...
  useEffect(() => {
    const fetchCustomerData = async () => {
      if (values.Customer?.WIC_ID) {
        ...
        if (notifyParties && notifyParties.length > 0) {
          // In Edit Mode: Check if we should preserve the existing value
          const existingNotify =
            values.Customer.WIC_ID === currentData?.Customer?.WIC_ID
              ? notifyParties.find((p) => p.NotifyPartyID === currentData?.NotifyPartyID)
              : null;

          if (existingNotify) {
            setValue('NotifyParty', existingNotify);
          } else {
        ...
        if (consignees && consignees.length > 0) {
          // In Edit Mode: Check if we should preserve the existing value
          const existingConsignee =
            values.Customer.WIC_ID === currentData?.Customer?.WIC_ID
              ? consignees.find((c) => c.ConsigneeID === currentData?.ConsigneeID)
              : null;

          if (existingConsignee) {
            setValue('Consignee', existingConsignee);
          } else {
    ...
    GetConsignees,
    setValue,
    currentData?.Customer?.WIC_ID,
    currentData?.NotifyPartyID,
    currentData?.ConsigneeID,
  ]);
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    // Check if we have currentData with ExportLCID for update
    if (!currentData?.ExportLCID) {
      enqueueSnackbar('Cannot update: Missing Export LC ID', { variant: 'error' });
      return;
    }

    const exportLCData = {
      // Include the ExportLCID for update
      ExportLCID: currentData.ExportLCID,
      ...
      PortID: data.Portofloading?.PortID || 0,
      ...
      ModifiedBy: userData?.userDetails?.userId,
    ...
    try {
      ...
      const response = await Put('CommercialModule/UpdateExportLC', exportLCData);

      if (response.status === 200) {
        enqueueSnackbar('Export L/C Updated Successfully!');
        router.push(paths.dashboard.Commercial.export.ExportLC.root);
      } else {
        enqueueSnackbar('Failed to update Export L/C!', { variant: 'error' });
      }
    } catch (error) {
      console.error('Export L/C Update Error:', error);
      enqueueSnackbar('Failed to update Export L/C!', { variant: 'error' });
    }
  });
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {selectedRows.map((row, index) => {
                            const isFromCurrentData = currentData?.Details?.some(
                              detail => getRowId(detail) === getRowId(row)
                            );
                            return (
                              <TableRow key={index}>
                                <TableCell>
                                  <IconButton color="error" onClick={() => handleRemoveRow(row)} disabled={isFromCurrentData}>
