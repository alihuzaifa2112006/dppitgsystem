...
import StaffCategoryTableRow from '../staffCategory-table-row';
import StaffCategoryTableToolbar from '../staffCategory-toolbar';
import StaffCategoryTableFiltersResult from '../staffCategory-filters-result';
...
const TABLE_HEAD = [
  { id: 'Category_Name', label: 'Staff Category Name', minWidth: 160 },
...
export default function StaffCategoryListView() {
  ...
  const FetchStaffCategory = useCallback(async () => {
    try {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchStaffCategory(), FetchAllRoles(), FetchAllUsers()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchStaffCategory, FetchAllRoles, FetchAllUsers]);
  ...
  const moveToEditForm = (row) => {
    if (allUsers?.flatMap((role) => role?.Staff_Cat_ID).includes(row.Staff_Cat_ID)) {
      enqueueSnackbar('This staff category is in use', { variant: 'error' });
      return;
    }
  ...
  const DeleteDetailTableRow = async (row) => {
    ...
    if (AllRoles?.flatMap((roles) => roles?.STAFF_CAT_ID).includes(row.Staff_Cat_ID)) {
      enqueueSnackbar('This staff category is being used in role database', { variant: 'error' });
      return;
    }
    try {
      await Delete(`DeleteStaffCategory/${row.Staff_Cat_ID}`);
    ...
    <>
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            heading="Staff Category"
            links={[
              { name: 'Home', href: paths.dashboard.root },
              { name: 'Staff Category', href: paths.dashboard.admin.staff.root },
              { name: 'list' },
            ]}
            action={
              <Button
          ...
          <Card>
            ...
            {canReset && (
              <StaffCategoryTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
                //
                ...
                >
                  ...
                  <TableBody>
                    {dataFiltered
                      ...
                      .map((row) => (
                        <StaffCategoryTableRow
                          key={row?.Staff_Cat_ID}
                          row={row}
                          selected={table.selected.includes(row?.Staff_Cat_ID)}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) => yarn?.Category_Name.toLowerCase().indexOf(name.toLowerCase()) !== -1
