...
import SectionTableToolbar from '../section-toolbar';
import SectionTableFiltersResult from '../section-filters-result';
import { alpha, Tab, Tabs } from '@mui/material';
import Label from 'src/components/label';
import SectionDialog from '../AddDialog';
import SectionEditDialog from '../EditDialog';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'SectionName', label: 'Section Name', minWidth: 160 },
...
export default function SectionListView() {
  ...
  const FetchSection = useCallback(async () => {
    try {
      const response = await Get(
        `HRModule/GetSectionList?Org_ID=${userData?.userDetails?.orgId}&Branch_ID=${userData?.userDetails?.branchID}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchSection(), FetchCity()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchSection, FetchCity]);
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Section', href: paths.dashboard.HR_Module.Setup.section },
              { name: 'list' },
            ]}
            action={
              <Button
