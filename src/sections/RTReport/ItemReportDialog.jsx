    ...
    try {
      ...
      const response = await axios.get(
        `${APP_API}ExportProductionReport?OrgId=${
          ...
          startDate,
          'yyyy-MM-dd'
        )}&toDate=${fDate(endDate, 'yyyy-MM-dd')}&DeptID=9&SectionID=29`,
        { responseType: 'blob' }
      );
      // hASHAM BHi ny marsgaa mai margasa nky mutakbiq section or dp id rakh wahi hai 
      // Verify the Blob
      ...
      link.href = blobUrl;
      link.download = `MargasaItemProductionReport_${fDate(startDate, 'dd-MM-yyyy')}_to_${fDate(endDate, 'dd-MM-yyyy')}${new Date().getTime()}.xlsx`;
