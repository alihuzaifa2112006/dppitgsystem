...
import HolidayTableToolbar from '../holiday-toolbar';
import HolidayTableFiltersResult from '../holiday-filters-result';
import { alpha, Tab, Tabs } from '@mui/material';
import Label from 'src/components/label';
import HolidayDialog from '../AddDialog';
import HolidayEditDialog from '../EditDialog';
import EmployeeEditDialog from '../EmployeeDialog';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'HolidayName', label: 'Holiday Name', minWidth: 160 },
  { id: 'HolidayDate', label: 'Holiday Date', minWidth: 140 },
  { id: 'BusniessYearName', label: 'Business Year', minWidth: 140 },
  { id: 'Comments', label: 'Comments', minWidth: 200 },
  { id: '', label: 'Add Employee', minWidth: 150, align: 'center' },
...
const [employeeEditDialogOpen, setEmployeeEditDialogOpen] = useState(false);
  ...
  const handleEmployeeEditDialogOpen = (rowData) => {
    setEmployeeEditDialogOpen(true);
    setCurrentRowData(rowData);
  };

  const handleEmployeeEditDialogClose = () => {
  ...
  const FetchHoliday = useCallback(async () => {
    try {
      const response = await Get(`HRModule/GetPublicHolidayList?Org_ID=${userData?.userDetails?.orgId}&Branch_ID=${userData?.userDetails?.branchID}`);
  ...
  useEffect(() => {
    const fetchData = async () => {
        await Promise.all([FetchHoliday(), FetchCity()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchHoliday, FetchCity]);
  ...
  const moveToEditForm = (e) => {
   ...
   const moveToEmployeeEditForm = (e) => {
    handleEmployeeEditDialogOpen(e);
    // navigate(paths.dashboard.customer.wic.edit(e));
  };
  const DeleteDetailTableRow = async (row) => {
    if (allCities.some((city) => city.Country_ID === row.Country_ID)){
      enqueueSnackbar('This country is in use!', { variant: 'error' });
      return;
    }
    try {
  ...
  return (
    <>
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Holiday', href: paths.dashboard.HR_Module.Setup.holidays },
              { name: 'list' },
            ]}
            action={
              <Button
                    ...
                    {dataFiltered
                      ...
                      .map((row) => (
                        <HolidayTableRow
                          ...
                          EmployeeRow={() => moveToEmployeeEditForm(row)}
                          onDeleteRow={() => {
      ...
      />
       <EmployeeEditDialog
        uploadClose={handleEmployeeEditDialogClose}
        row={currentRowData}
        uploadOpen={employeeEditDialogOpen}
        tableData={tableData}
      />
    </>
  );
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
        yarn?.HolidayName.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        yarn?.HolidayDate.toLowerCase().indexOf(name.toLowerCase()) !== -1
  ...
  if (role.length) {
    inputData = inputData.filter((yarn) => role.includes(yarn?.Country_Code));
  }
