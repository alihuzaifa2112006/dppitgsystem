  ...
  const InsertVendor = useCallback(
    async (fileData) => {
      ...
      try {
        ...
        const response = await Post(`registerWICMultiple`, modifiedVendorData);

        if (response.status === 200) {
          enqueueSnackbar('Leads Added successfully!', { variant: 'success' });
        }
        uploadClose();
        FetchUpdatedData();
      } catch (error) {
          ...
          <Grid container>
            <Grid xs={12} md={12}>
              <DialogContent>
                ...
                <a href="https://apicyclo.scmcloud.online/Uploads/PreOnboardCustomer.xlsx" download>
                  <Button
