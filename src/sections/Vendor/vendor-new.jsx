...
export default function ProductCreateForm() {
  ...
  const fetchOrigin = useCallback(async () => {
    if (selectedSource?.SourceID) {
      try {
        ...
        if (selectedSource?.SourceID === 1) {
          setValue('Origin', response.data[0]);
        } else {
  ...
  const onSubmit = handleSubmit(async (data) => {
    const dataToSend = {
      InventoryTypeID: data?.InvType?.ClassID || 0,
      VendorTypeID: data?.VendorType?.VendorTypeID || 0,
    ...
    try {
      const res = await Post('AddVendor', dataToSend);
      ...
      if (res?.status === 409) {
        enqueueSnackbar('Vendor Name Exists', { variant: 'error' });
      } else {
        enqueueSnackbar('Vendor Creation Failed', { variant: 'error' });
      }
    } catch (error) {
            ...
            <Card sx={{ p: 3 }}>
              ...
              >
                ...
                <AutocompleteWithAdd
                  ...
                  disabled={values?.Source?.SourceID === 1}
