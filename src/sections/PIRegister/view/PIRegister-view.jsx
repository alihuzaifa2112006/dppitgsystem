...
import { decrypt, encrypt } from 'src/api/encryption';
  ...
  import PIRegisterTableRow from '../PIRegister-table-row';
  import PIRegisterTableToolbar from '../PIRegister-toolbar';
  import PIRegisterTableFiltersResult from '../PIRegister-filters-result';
import axios from 'axios';

// ----------------------------------------------------------------------

const TABLE_HEAD = [
  { id: 'PIRegisterNo', label: 'PI Register No.', minWidth: 145 },
...
export default function PIRegisterListView() {
  ...
  const moveToEditForm = (e) => {
    navigate(paths.dashboard.Commercial.import.ImportPIRegister.edit(e));
  };
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Import PI Register', href: paths.dashboard.Commercial.import.ImportPIRegister.root },
              { name: 'List' },
            ]}
            action={
              <Button
                ...
                >
                  ...
                  <TableBody>
                    {dataFiltered
                      ...
                      .map((row) => (
                          <PIRegisterTableRow
                          key={row?.PIRegisterID}
                          row={row}
                          selected={table.selected.includes(row?.PIRegisterID)}
                          onEditRow={() => moveToEditForm(row?.PIRegisterID)}
                        />
                      ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) =>
          yarn?.PIRegisterNo.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
