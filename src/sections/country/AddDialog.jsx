...
export default function CountryDialog({ uploadClose, uploadOpen, tableData }) {
  ...
  const PostCountryData = async (PostData) => {
    try {
      await Post('country', PostData).then(async (res) => {
        enqueueSnackbar(res.data.Message, "country  Added ");
          uploadClose();
  ...
  const onDptSubmit = handleSubmit(async (data) => {
    if (tableData.some((item) => item.Country_Name === data.Country)) {
      enqueueSnackbar('Country Name already exists', { variant: 'error' });
      return;
    }
    try {
      const dataToSend = {
        ...
        <DialogContent sx={{ py: 3 }}>
          <FormProvider methods={methods} onSubmit={onDptSubmit}>
            ...
            >
              ...
              <RHFTextField name="Country" label="Country Name" />
            </Box>
