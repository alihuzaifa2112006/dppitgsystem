...
const TABLE_HEAD = [
  { id: 'Country_Name', label: 'Country Name', minWidth: 160 },
...
export default function CountryListView() {
  ...
  const FetchCountry = useCallback(async () => {
    try {
      const response = await Get(`getAllActiveInactivecountries`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchCountry(), FetchCity()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchCountry, FetchCity]);
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Country', href: paths.dashboard.powertools.country },
              { name: 'list' },
            ]}
            action={
              <Button
