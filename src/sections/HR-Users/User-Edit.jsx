...
export default function UserEditForm({ currentUser }) {
    ...
    const defaultValues = useMemo(() => ({
        // Step 1
        machineCode: currentUser?.EmployeeMachineCode || '',
        fullName: currentUser?.EmployeeName || '',
        fathersName: currentUser?.FatherName || '',
        mothersName: currentUser?.MotherName || '',
        religion: currentUser?.Religion || '',
        nationality: currentUser?.Nationality || '',
        dateOfBirth: currentUser?.DateofBirth ? new Date(currentUser.DateofBirth) : null,
        maritalStatus: currentUser?.MaritalStatusID ? MartialStatus.find(i => i.id === currentUser.MaritalStatusID) : null,
        gender: currentUser?.GenderID ? Gender.find(i => i.id === currentUser.GenderID) : null,
        department: currentUser?.DepId ? { DepId: currentUser.DepId, DepartmentName: currentUser.DepartmentName } : null,
        section: currentUser?.SectionID ? { SectionID: currentUser.SectionID, SectionName: currentUser.SectionName } : null,
        designation: currentUser?.DesigId ? { DesigId: currentUser.DesigId, DesignationName: currentUser.DesignationName } : null,
        placeOfBirth: currentUser?.PlaceofBirth || '',
        email: currentUser?.Email || '',
        contactNo: currentUser?.CellNo || '',
        bloodGroup: currentUser?.BloodGroupID ? bloodGroupOptions.find(i => i.BloodGroupID === currentUser.BloodGroupID) : null,
        gradeNo: currentUser?.Grade_ID ? gradeOptions.find(i => i.Grade_ID === currentUser.Grade_ID) : null,
        jobLocation: currentUser?.JobLocationInfoID ? jobLocationOptions.find(i => i.JobLocationInfoID === currentUser.JobLocationInfoID) : null,
        photo: currentUser?.Photo || null,

        // Step 2
        presentAddress: currentUser?.PresentAddress || '',
        permanentAddress: currentUser?.PermanentAddress || '',
        presentState: currentUser?.PresentState || '',
        permanentState: currentUser?.PermanentState || '',
        presentZipCode: currentUser?.PresentZipCode || '',
        permanentZipCode: currentUser?.PermanentZipCode || '',
        presentDistrict: currentUser?.PresentDistrict || '',
        permanentDistrict: currentUser?.PermanentDistrict || '',
        ...
        nextToKin: currentUser?.NextToKin || '',
        nid: currentUser?.CNIC || '',
        nidExpiryDate: currentUser?.CNICExpiryDate ? new Date(currentUser.CNICExpiryDate) : null,
        insurancePolicy: currentUser?.InsurancePolicy || '',
        ntnNo: currentUser?.NTNNo || '',

        // Step 4
        dateOfAppointment: currentUser?.AppointmentDate ? new Date(currentUser.AppointmentDate) : null,
        employeeCategory: currentUser?.EmployeeCategoryID ? employeeCategoryOptions.find(i => i.id === currentUser.EmployeeCategoryID) : null,
        staffCategory: currentUser?.StaffCategoryID ? staffCategoryOptions.find(i => i.StaffCategoryID === currentUser.StaffCategoryID) : null,
        shift: currentUser?.ShiftId ? shiftOptions.find(i => i.id === currentUser.ShiftId) : null,
        accountNo: currentUser?.AccountNo || '',
        bankBranch: currentUser?.BankBranchName || '',
        paymentType: currentUser?.EmployeePaymentTypeID ? paymentTypeOptions.find(i => i.EmployeePaymentTypeID === currentUser.EmployeePaymentTypeID) : null,
        status: currentUser?.IsActive !== undefined ? (currentUser.IsActive === 'active' || currentUser.IsActive === true ? { id: 1, name: 'Active' } : { id: 2, name: 'InActive' }) : statusOptions[0],

        // Step 5
        nomineeName: currentUser?.NomineeName || '',
        nomineeDob: currentUser?.NomineeDOB ? new Date(currentUser.NomineeDOB) : null,
        nomineeNid: currentUser?.NomineeNIDNo || '',
        nomineeRelationship: currentUser?.NomineeRelationship || '',
        nomineeAddress: currentUser?.NomineeAddress || '',
        nomineeMobileNo: currentUser?.NomineeMobileNo || '',
    }),
        // eslint-disable-next-line 
        [currentUser, bloodGroupOptions, gradeOptions, jobLocationOptions, staffCategoryOptions, shiftOptions, paymentTypeOptions]);
    ...
    const onSubmit = handleSubmit(async (data) => {
        try {
            ...
            let response;
            if (currentUser) {
                employeeData.HRID = currentUser.HRID;
                // Re-append updated employeeData for update request
                formData.set('employee', JSON.stringify(employeeData));
                response = await Put('HRModule/UpdateEmployeeProfile', formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });
            } else {
                response = await Post('HRModule/CreateEmployeeProfile', formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });
            ...
            if (response.status === 200 || response.status === 201) {
                enqueueSnackbar(currentUser ? 'Employee updated successfully!' : 'Employee created successfully!', { variant: 'success' });
                router.push(paths.dashboard.HR_Module.HR_Users.root);
            } else {
                throw new Error(currentUser ? 'Failed to update employee' : 'Failed to create employee');
            }
        } catch (error) {
            console.error('Save Error:', error);
            enqueueSnackbar(
                error.response?.data?.message || error.message || (currentUser ? 'Error updating employee' : 'Error creating employee'),
                { variant: 'error' }
            );
        }
    });
    ...
    const renderNomineeDetails = () => (
        <Card sx={{ p: 3, mt: 3 }}>
            ...
            >
                ...
                <RHFTextField name="nomineeMobileNo" label="Mobile No" fullWidth key='20' />
            </Box>
        </Card>
    );
