...
import DepartmentTableToolbar from '../department-toolbar';
import DepartmentTableFiltersResult from '../department-filters-result';
...
const TABLE_HEAD = [
  { id: 'Dpt_Name', label: 'Department Name', minWidth: 160 },
...
export default function DepartmentListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchDepartment(), FetchAllRoles(), FetchAllSections()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchDepartment, FetchAllRoles, FetchAllSections]);
  ...
  const DeleteDetailTableRow = async (row) => {
    if (AllRoles?.some((role) => role.DPT_ID === row.Dpt_ID)) {
      enqueueSnackbar('This department is being used in role database', { variant: 'error' });
      return;
    }
    if (AllSections?.some((role) => role.DPT_ID === row.Dpt_ID)) {
      enqueueSnackbar('This department is being used in section database', { variant: 'error' });
      return;
    }
    try {
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Department', href: paths.dashboard.admin.department.root },
              { name: 'list' },
            ]}
            action={
              <Button
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (yarn) => yarn?.Dpt_Name.toLowerCase().indexOf(name.toLowerCase()) !== -1
