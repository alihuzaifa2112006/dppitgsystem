...
import ProductionBucketTableRow from '../ProductionBucket-table-row';
import ProductionBucketTableToolbar from '../ProductionBucket-toolbar';
import ProductionBucketTableFiltersResult from '../ProductionBucket-filters-result';
...
export default function ProductionBucketListView() {
  ...
  const FetchProductionBucketData = useCallback(async () => {
    try {
      const response = await Get(
        `GetAllInvProductionBucketing?orgId=${userData?.userDetails?.orgId}&branchId=${userData?.userDetails?.branchID}`
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      await Promise.all([FetchProductionBucketData()]);
      setLoading(false);
    };
    fetchData();
  }, [FetchProductionBucketData]);
  ...
  const moveToEditForm = (e) => {
    navigate(paths.dashboard.Production.Planning.ProductionBucket.edit(e));
  };
  // Approval Functions
  const moveToApprovalForm = (e) => {
    navigate(paths.dashboard.Production.Planning.ProductionBucket.approval(e));
  };
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          <CustomBreadcrumbs
            ...
            links={[
              ...
              { name: 'Item Entry', href: paths.dashboard.Production.Planning.ProductionBucket.root },
              { name: 'List' },
            ]}
            action={
              <Button
                component={RouterLink}
                href={paths.dashboard.Production.Planning.ProductionBucket.new}
                variant="contained"
          ...
          <Card>
            ...
            {canReset && (
              <ProductionBucketTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
                //
